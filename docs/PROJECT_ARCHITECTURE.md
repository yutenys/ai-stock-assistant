...
> 文档对应当前 Electron 工程。应用以公开行情接口和本地确定性规则为主；可选 AI 只能引用冻结证据生成解释，不参与补数和评分。分析结果只用于条件观察，不构成投资建议。
...
flowchart LR
  ...
  subgraph Main["Main 主进程"]
    ...
    Workflow["通用行业工作流"]
    Quotes["行情与资金聚合"]
    Analysis["横截面因子 / 独立策略 / 统一裁决"]
    Overview["大盘快照 / 可恢复全市场任务"]
    Profile["公司资料与资讯"]
    Cache["内存缓存 / 磁盘缓存"]
    Log["每日操作日志"]
  end
...
├─ lib/
│  ├─ persistence.js       原子JSON、备份恢复、后台任务游标
│  ├─ strategy-platform.js 数据契约、横截面因子、策略注册表和统一裁决
│  └─ research-account.js  独立研究账户、费用、T+1和集中度
...
| `labels` | 用户标签及标签内股票 | 是，主进程文件；兼容旧 `localStorage` |
| `activeLabel` | 当前标签 | 是 |
| `portfolio` | 模拟持仓数量、加权成本、已实现收益和最近行情快照 | 是，主进程文件 |
| `simulatedTrades` | 完整模拟买卖成交账本；页面可按需截取展示 | 是，主进程文件，不裁剪账本 |
| `researchAccount` | 独立研究账户、订单、成交、现金、持仓和费用规则 | 是，独立主进程文件 |
| `detailHistoryCache` | 当前进程内技术分析结果 | 否 |
| `lastStatusNotice` | 角落状态提示 | 否 |

生成股票池和线上搜索只清理主显示区，不删除标签。股票保存到新标签时采用增量合并，同一股票可同时存在于多个标签。

旧模拟交易按 A 股 100 股整数手计算并保留原口径。研究账户与旧模拟持仓隔离，显式记录初始资金、佣金、最低佣金、印花税、过户费、T+1、成交和拒绝原因；两者均按最新可用行情估值，持仓页展示行业集中度。
...
| `start/get/cancel-full-market-research` | 启动、订阅、取消或恢复全市场历史后台任务 |
| `load/save-user-state` | 用户标签、旧模拟持仓和完整账本的原子持久化 |
| `load/create/mark-research-account` | 独立研究账户创建、读取和最新行情估值 |
| `execute-research-order` | 按显式费用、整手、现金和T+1执行研究委托 |
| `explain-stock-with-ai` | 可选AI证据解释；输出必须通过证据编号校验 |
...
- 页面刷新保留快速候选路径，保证大盘交互不会等待整市场历史下载。
- 独立后台任务遍历当天证券快照，逐股采集最多320根日线，成功结果按代码幂等保存，失败记录在恢复时重试。
- 任务完成后发布带交易日、模型版本、全市场数量、历史有效数和失败数的因子快照；历史不足的股票保持缺失状态，不伪造RPS。
- 首页只限制预览数量，“查看更多”和数据层不截断已合格结果；优先级只改变处理顺序，不改变股票是否被评估。
...
大盘推荐在技术形态、质量闸门、消息面和未来半年风险核验基础上，增加可验证多因子评分：估值质量（PE/PB）、供需量能（量比、换手、成交额）、板块领先（同板块涨幅、上涨占比和成交活跃度）、市场方向（全市场上涨/下跌宽度）以及成长/创新催化（可核验新闻）。该框架参考用户提供图片中的 CANSLIM 和估值分析思路，但只计算当前公开接口实际返回的数据。

季度/年度 EPS 增长、ROE、现金流和机构持仓变化在现有数据源缺乏可追溯序列时显示“数据不足”，不计分也不扣分，禁止用估算值补齐。每只推荐展示多因子分和数据覆盖项数，原因中注明板块强度。

首页前 10 条和“查看更多/一键保存”列表均按板块聚合，同一板块连续显示；板块标题展示强势/活跃/中性/偏弱标签、板块评分和股票数量，每只股票名称旁高亮显示板块标签。
...
- 开发运行时数据根目录是项目目录；安装包使用 Electron 可写的 `userData` 目录，避免向只读安装目录写入。
- 重要JSON通过同卷临时文件原子重命名，并保留上一版 `.bak`；主文件损坏时读取备份并向界面返回恢复状态。
- `data/user-state.json`：标签、旧模拟持仓与完整成交账本。
- `data/research-account.json`：独立研究账户。
- `data/full-market-jobs/`：全市场任务清单和逐股结果。
...
- 大盘结果分为严格推荐、观察候选、强势追踪三个独立池；首页只限制预览，“查看更多”展示当前冻结分析版本的全部结果并支持批量保存。
...
- 后台全市场任务支持启动、取消、恢复、失败重试和真实覆盖率；大盘页面刷新不等待该任务结束。
- 推荐与详情复用同一策略裁决和 `analysisId`，收盘确认必须同时具备交易日、源时间和最终日线证据。

## 14. CANSLIM 与价值质量升级

- 大盘推荐保持“技术形态先初筛、财务与风险再精筛”，只对历史精筛候选请求东方财富 F10 财务数据，并缓存 6 小时，避免全市场逐股请求。
- CANSLIM 使用 C/A/N/S/L/I/M 七维展示：C/A 取季度和年度 EPS、净利润、营收、ROE；S 取量比、换手和成交额；L 取近 60 日相对强度与板块强度；N 取可核验消息；M 取市场宽度；I 在机构持仓序列未接入时明确显示数据不足。
- 七维得分只按可核验维度归一化，同时展示覆盖数和原始可得分，不把缺失维度当成零分或通过项。
- 价值质量检查使用 ROE、每股经营现金流、流动比率、资产负债率、毛利率、净利率和年度现金流持续性。最近报告期营收、利润、ROE、现金流出现组合恶化时，候选会被风险闸门排除。
- 个股详情新增财务季度表、CANSLIM 逐维证据、价值质量、PE/PB 和财务红旗。DCF 与护城河缺少真实预测和竞争结构证据时只显示数据缺口，不自动生成目标价或高分。
- 操作计划继续使用 ATR、均线和支撑位，并增加 10 日低点与初始最大 8% 止损距离约束；所有价格仍是条件演算，不构成收益承诺。
