...
function evaluateStrategyRegistry(factor, context = {}) {
  ...
  const firstPullback = pullback.confirmed && pullback.barsSinceBreakout >= 1 && pullback.barsSinceBreakout <= 15
    && pullback.priorPullbackCount === 0 && factor?.close >= factor?.ma20 * .98 && factor?.close <= factor?.ma20 * 1.04
...
module.exports = {createDataEnvelope, percentile, detectBreakoutContext, buildFactorSnapshot, evaluateStrategyRegistry, arbitrateStrategyResults, buildAnalysisViewModel, mergeEventLifecycle};
