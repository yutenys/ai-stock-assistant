...
function factorBase(row) {
  ...
  return {
    code:String(row.code || '').slice(-6), close, high20, low20,
...
function evaluateStrategyRegistry(factor, context = {}) {
  ...
  return [
    result('trend-breakout', nearHigh && volumeControlled && trend && rps20 >= 80,
      55 + (rps20 || 0) * .25 + (rps60 || 0) * .1, '突破确认', '短线', ['接近20日高点', '量比处于1.5至4倍', '均线趋势向上'], ['price-20d','volume-20d','ma-20-30']),
