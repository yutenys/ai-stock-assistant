...
class ResearchJobStore {
  ...
  async start(input) {
    ...
    const manifest = {...input, state:'running', cursor:0, completed:0, failed:0, createdAt:now, updatedAt:now};
  ...
  async recordBatch(jobId, rows) {
    ...
    const next = {...manifest, cursor:results.length, completed:results.length, updatedAt:new Date().toISOString()};
