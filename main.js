...
const RESEARCH_ACCOUNT_RULES = Object.freeze({
  ...
  minimumCommission:5,
  stampDutyRate:.0005,
  transferFeeRate:.00001,
  lotSize:100,
  tPlusOne:true,
  effectiveFrom:'2023-08-28',
  source:'研究回放默认费用假设，使用前需按实际券商与交易所规则复核'
});
...
function logFilePath() {
  ...
  return path.join(dataRootPath(), 'logs', `${date}.log`);
}

function cacheFilePath(name) {
  return path.join(dataRootPath(), 'cache', `${name}.json`);
}
...
function readDiskCache(name, maxAgeMs = 7 * 24 * 60 * 60 * 1000) {
  try {
    ...
    try {
      ...
      parsed = JSON.parse(fs.readFileSync(`${file}.bak`, 'utf8'));
    }
...
function writeDiskCache(name, data) {
  const file = cacheFilePath(name);
  atomicWriteJson(file, { savedAt: Date.now(), data }).catch(error => {
    appendLogLine({type:'error', action:'disk_cache_write_failed', message:`缓存写入失败：${name}`, stack:error.stack || String(error)});
  });
}
...
function appendRecommendationLedger(result) {
...
function requestText(url, timeout = 12000, extraHeaders = {}, encoding = 'utf8', redirectsLeft = 3) {
  return new Promise((resolve, reject) => {
    ...
    const req = client.get(url, {
      ...
      const chunks = [];
      res.on('error', reject);
      res.on('aborted', () => reject(new Error('行情响应中途断开')));
...
function chinaClockParts(value = Date.now()) {
...
function resolveObservationPhase(tradeDate, observedAt = Date.now(), { sourceObservedAt = null, isFinalBar = null } = {}) {
  ...
  const phase = minutes < 9 * 60 + 15 ? 'preopen'
    : !clockClosed || isFinalBar === false || sourceStillIntraday ? 'intraday'
      : isFinalBar === true || sourceMinutes !== null ? 'closed' : 'unknown';
  const label = phase === 'preopen' ? '开盘前'
    : phase === 'intraday' ? '盘中快照'
      : phase === 'closed' ? '当日收盘' : '收盘待确认';
  return {phase, label, tradeDate:date, observedAt:new Date(observedAt).toISOString()};
}

function buildAiEvidencePrompt({code = '', analysisId = '', evidence = []} = {}) {
  const rows = (evidence || []).filter(item => item?.id && item?.text).map(item => `[${item.id}] ${item.text}`);
  return `股票代码：${code}\n分析快照：${analysisId}\n证据：\n${rows.join('\n')}\n\n请只依据以上证据解释趋势、资金、消息冲突和条件化风险。每个判断必须引用证据编号，不给保证性结论或确定目标价。`;
}

function validateAiExplanation(text, evidenceIds = []) {
  const allowed = new Set(evidenceIds.map(String));
  const references = [...String(text || '').matchAll(/\[([^\]]+)\]/g)].map(match => match[1]);
  const unsupported = references.filter(id => !allowed.has(id));
  return {valid:Boolean(String(text || '').trim()) && references.length > 0 && unsupported.length === 0, references, unsupported};
}

function postJson(url, body, headers = {}, timeout = 30000) {
  return new Promise((resolve, reject) => {
    const target = new URL(url);
    if (target.protocol !== 'https:') return reject(new Error('AI服务地址必须使用 HTTPS'));
    const payload = Buffer.from(JSON.stringify(body));
    const req = https.request(target, {method:'POST', timeout, headers:{'Content-Type':'application/json','Content-Length':payload.length,...headers}}, response => {
      const chunks = [];
      response.on('data', chunk => chunks.push(chunk));
      response.on('end', () => {
        const text = Buffer.concat(chunks).toString('utf8');
        if (response.statusCode < 200 || response.statusCode >= 300) return reject(new Error(`AI服务 HTTP ${response.statusCode}：${text.slice(0, 200)}`));
        try { resolve(JSON.parse(text)); } catch { reject(new Error('AI服务返回非 JSON 数据')); }
      });
    });
    req.on('timeout', () => req.destroy(new Error('AI解释请求超时')));
    req.on('error', reject);
    req.end(payload);
  });
}
...
async function fetchQuoteRows(codes) {
  const fields = 'f12,f14,f2,f3,f4,f5,f6,f7,f8,f9,f10,f15,f16,f17,f18,f20,f21,f23,f51,f52,f62,f64,f65,f70,f71,f184,f57,f58,f124,f267,f268,f164,f165,f174,f175';
  const secids = codes.map(secidOf).join(',');
  const pathname = `/api/qt/ulist.np/get?fltt=2&fields=${fields}&secids=${encodeURIComponent(secids)}&_=${Date.now()}`;
  const { json } = await fetchEastmoneyHostPath(pathname, 3500);
  return eastmoneyListRows(json.data);
}

function normalizeEastmoneyFundFlow(data) {
  const sum = (a, b) => finiteNumber(a) !== null && finiteNumber(b) !== null ? Number(a) + Number(b) : null;
  return {
    mainInflow: sum(data.f64, data.f70),
    mainOutflow: sum(data.f65, data.f71),
    mainNetInflow: finiteNumber(data.f62),
    mainNetPct: finiteNumber(data.f184),
    ...capitalWindowFields(data),
...
async function fetchEastmoneySingleFundFlow(code) {
  ...
  return {
    ...normalizeEastmoneyFundFlow(data),
    source: '东方财富单股资金',
    tradeDate: data.f124 > 0 ? new Date(data.f124 * 1000).toISOString().slice(0, 10) : '',
    estimated: false
  };
...
async function fetchTencentMarketSnapshot() {
  ...
  return {
    ...
    limits: {
      ...
      observationReason:'涨停和跌停仅用于情绪与梯队观察；现有快照不含首封时间、炸板次数和连续板数，不生成买入许可。',
      dataCoverage:{universe:quotes.length, scope:'腾讯全市场行情快照', observedAt:new Date().toISOString()}
    },
    errors
  };
}

async function fetchTencentMarketSectors(marketQuotes, capitalRows = []) {
  ...
  try {
    const liveDirectory = await fetchStockDirectory();
    if (liveDirectory.length) directory = liveDirectory;
  } catch {}
  const directoryByCode = new Map(directory.map(item => [String(item.code || ''), item]));
  const rotation = buildIndustryRotationFromQuotes(marketQuotes, directoryByCode);
  if (rotation.sectors.length < 8) return broadResult;
  const merged = mergeSectorCapitalRows(rotation, capitalRows, marketQuotes);
  return { ...merged, broadSectors:broadResult.sectors };
}

function buildIndustryRotationFromQuotes(marketQuotes, directoryByCode = null) {
  const groups = new Map();
  (marketQuotes || []).forEach(quote => {
    const industry = resolveRecommendationIndustry(quote, directoryByCode);
    if (industry === '行业待确认') return;
    if (!groups.has(industry)) groups.set(industry, []);
    groups.get(industry).push(quote);
  });
  const baseRows = [...groups.entries()].filter(([, stocks]) => stocks.length >= 5).map(([name, stocks]) => {
    ...
    const amountPerStock = stocks.length ? amount / stocks.length : 0;
    ...
    const leaders = stocks.filter(item => finiteNumber(item.changePct) !== null)
      .sort((a, b) => Number(b.changePct) - Number(a.changePct) || Number(b.amount || 0) - Number(a.amount || 0));
    return { name, count: stocks.length, changePct: averageChangePct, averageChangePct, upRatio, amount, amountPerStock, leader: leaders[0]?.name || '', representatives: leaders.slice(0, 3).map(item => item.name) };
  });
  const liquidityValues = baseRows.map(item => item.amountPerStock);
  const sectors = baseRows.map(item => {
    const liquidityScore = percentileScore(liquidityValues, item.amountPerStock);
    const rotationScore = clampRecommendationScore(35 + item.averageChangePct * 8 + (item.upRatio - .5) * 45 + liquidityScore * .2);
    const rotationState = rotationScore >= 72 ? '资金升温' : rotationScore >= 58 ? '轮动活跃' : rotationScore <= 38 ? '资金退潮' : '轮动中性';
    return { ...item, liquidityScore, rotationScore, rotationState, source: '全市场行业涨幅、上涨广度与成交活跃度' };
  });
  return {
    sectors: [...sectors].sort((a, b) => b.rotationScore - a.rotationScore || b.changePct - a.changePct),
    weakSectors: [...sectors].sort((a, b) => a.rotationScore - b.rotationScore || a.changePct - b.changePct).slice(0, 6),
    fundSectors: [...sectors].sort((a, b) => b.rotationScore - a.rotationScore || b.amountPerStock - a.amountPerStock)
  };
}

function normalizeSectorCapitalName(value) {
  return cleanStockName(value).replace(/[ⅠⅡⅢⅣⅤ]+$/u, '').replace(/[一二三四五]级$/u, '').trim();
}

function capitalWindowFields(row) {
  // Eastmoney's official 3/5/10-day capital table reports amounts in yuan.
  return {mainNet3:finiteNumber(row.f267), mainNetPct3:finiteNumber(row.f268),
    mainNet5:finiteNumber(row.f164), mainNetPct5:finiteNumber(row.f165),
    mainNet10:finiteNumber(row.f174), mainNetPct10:finiteNumber(row.f175)};
}

function isSectorRotationTheme(row) {
  if (row.boardType !== 'concept') return true;
  const name = normalizeSectorCapitalName(row.name);
  return !/^(融资融券|沪股通|深股通|机构重仓|基金重仓|社保重仓|QFII重仓|证金持股|转债标的|行业龙头|ST股|AB股|AH股|B股|沪深300|HS300|中证500|上证50|上证180|上证380|创业成份|创业板综|深成500|深证100R|MSCI中国|富时罗素)$/.test(name)
    && !/^\d{4}(?:[一二三四]季报|中报|年报)/.test(name)
    && !/^(?:昨日|最近|近期|百日|历史)(?:涨停|连板|触板|首板|炸板|高换手|高振幅|多板|打二板|新高)/.test(name)
    && !/^(?:微盘|小盘|中盘|大盘)(?:股|精选|成长|价值)$/.test(name);
}

function assessSectorCapitalTrend(row) {
  const net = finiteNumber(row.mainNetInflow), net3 = finiteNumber(row.mainNet3);
  const net5 = finiteNumber(row.mainNet5), net10 = finiteNumber(row.mainNet10);
  const valid = net !== null && !row.capitalEstimated && !row.capitalStale;
  const available = valid && (net5 !== null || net10 !== null);
  // Rolling windows overlap; subtraction isolates the earlier days.
  const prior4Net = net5 !== null && net !== null ? net5 - net : null;
  const prior5Net = net10 !== null && net5 !== null ? net10 - net5 : null;
...
function classifySectorRotationPhase(row) {
  ...
  const phase = trend.weakening ? '退潮'
    : trend.confirmed && crowded && breadth !== null && breadth >= 65 ? '拥挤加速'
      : trend.confirmed && concentrated ? '龙头集中'
        : trend.confirmed && breadth !== null && breadth >= 60 ? '扩散确认'
...
function eastmoneyListRows(data) {
  const rows = data?.diff;
  if (!rows || typeof rows !== 'object') return [];
  return (Array.isArray(rows) ? rows : Object.values(rows)).filter(row => row && typeof row === 'object');
}

function normalizeSectorCapitalRows(rows, boardType, host, fetchedAt = new Date().toISOString()) {
  const typeLabel = boardType === 'concept' ? '概念' : '行业';
  return (rows || []).map(row => ({
    code:String(row.f12 || ''),
    name:normalizeSectorCapitalName(row.f14),
    boardType,
    price:finiteNumber(row.f2),
    changePct:finiteNumber(row.f3),
    totalMarketCap:finiteNumber(row.f20),
    mainNetInflow:finiteNumber(row.f62),
    mainNetPct:finiteNumber(row.f184),
    ...capitalWindowFields(row),
    capitalTradeDate:row.f124 > 0 ? new Date(row.f124 * 1000).toISOString().slice(0, 10) : '',
    source:`东方财富${typeLabel}板块主力资金（${new URL(host).hostname}）`,
    fetchedAt
  })).filter(item => /^BK\d{4}$/.test(item.code) && item.name && item.mainNetInflow !== null)
    .sort((a, b) => Number(b.mainNetInflow) - Number(a.mainNetInflow))
    .map((item, index) => ({ ...item, capitalRank:index + 1 }));
}

async function fetchEastmoneyHostPath(pathname, timeout = 12000) {
  const errors = [];
  const hosts = ['https://push2.eastmoney.com', 'https://82.push2.eastmoney.com'];
  const routes = [...hosts.map(host => ({host, path:`/webguest${pathname}`})), ...hosts.map(host => ({host, path:pathname}))];
  for (const {host, path} of routes) {
    try {
      const json = JSON.parse(await requestText(`${host}${path}`, timeout));
      if (json?.rc !== 0 || !json.data || !json.data.diff) throw new Error('板块接口业务错误或缺少列表数据');
      return { json, host, errors };
    } catch (err) {
      errors.push(`${new URL(host).hostname}${path.split('?')[0]}: ${err.message || err}`);
    }
  }
  throw new Error(errors.join('；') || '东方财富板块接口不可用');
}

async function fetchEastmoneyListPages(params, timeout, fetchPage = fetchEastmoneyHostPath) {
  ...
  if (total === null || total < 0 || total > 10000) throw new Error('板块列表总数无效');
  const pages = Math.ceil(total / 100);
  const results = await settleWithConcurrency(Array.from({length:Math.max(0, pages - 1)}, (_, i) => i + 2), 3, async page => {
    const next = new URLSearchParams(query);
    next.set('pn', String(page));
    return fetchPage(`/api/qt/clist/get?${next}`, timeout);
  });
  for (const result of results) {
    if (result.status === 'rejected') throw result.reason;
    rows.push(...eastmoneyListRows(result.value.json?.data));
    first.errors.push(...result.value.errors);
  }
  const unique = [...new Map(rows.filter(row => row.f12).map(row => [String(row.f12), row])).values()];
  if (unique.length < total) throw new Error(`板块列表不完整：${unique.length}/${total}`);
  return {...first, rows:unique};
}

async function fetchSectorCapitalRank(boardType, sortField) {
  const boardFilter = boardType === 'concept' ? 'm:90+t:3' : 'm:90+t:2';
  const params = new URLSearchParams({
    pn:'1', pz:'100', po:'1', np:'1', fltt:'2', invt:'2', fid:sortField,
    fs:boardFilter, fields:'f12,f14,f2,f3,f20,f62,f184,f267,f268,f164,f165,f174,f175,f124', _:String(Date.now())
  });
  const response = await fetchEastmoneyListPages(params, 3500);
  const rows = normalizeSectorCapitalRows(response.rows, boardType, response.host);
  if (!rows.length) throw new Error(`东方财富${boardType === 'concept' ? '概念' : '行业'}板块资金返回为空`);
  return { rows, host:response.host, warnings:response.errors };
}

async function fetchSectorCapitalFlow(force = false) {
  if (!force && sectorCapitalCache && Date.now() - sectorCapitalCache.savedAt < 60 * 1000) {
    return { ...sectorCapitalCache.value, cached:true };
  }
  const previous = readDiskCache('sector-capital', SECTOR_CAPITAL_FALLBACK_MAX_AGE_MS);
  const requests = await Promise.allSettled([
    fetchSectorCapitalRank('industry', 'f62'),
    fetchSectorCapitalRank('concept', 'f62')
  ]);
  const warnings = [];
  ...
  requests.forEach(result => {
    ...
    hosts.add(new URL(result.value.host).hostname);
    warnings.push(...result.value.warnings);
    result.value.rows.forEach(row => {
      const current = byCode.get(row.code) || {};
      byCode.set(row.code, { ...current, ...row, capitalRank:Math.min(Number(current.capitalRank || Infinity), Number(row.capitalRank || Infinity)) });
    });
  });
  if (!byCode.size) {
    if (previous?.rows?.length) {
      const fallback = { ...previous, rows:previous.rows.map(row => ({...row, capitalStale:true})), cached:true, stale:true, warnings:[...(previous.warnings || []), ...warnings] };
      sectorCapitalCache = { savedAt:Date.now(), value:fallback };
      return fallback;
    }
    throw new Error(warnings.join('；') || '板块主力资金接口未返回数据');
  }
  ...
  const value = {
    rows,
    direct:true,
    source:`东方财富行业/概念板块主力资金（${[...hosts].join(' + ')}）`,
    fetchedAt:new Date().toISOString(),
    warnings:[...new Set(warnings)]
  };
  sectorCapitalCache = { savedAt:Date.now(), value };
  writeDiskCache('sector-capital', value);
  appendLogLine({
    type:warnings.length ? 'warn' : 'info', message:'板块主力资金加载完成', action:'sector_capital_loaded',
    detail:{ rows:rows.length, memberBoards:memberBoards.filter(item => item.memberCodes?.length).map(item=>({code:item.code,name:item.name,type:item.boardType,count:item.memberCodes.length})), hosts:[...hosts], warnings }
  });
  return value;
}

function selectSectorMemberBoards(rows, limit = 20) {
  const eligible = (rows || []).filter(row => isSectorRotationTheme(row) && !row.capitalStale && !assessSectorCapitalTrend(row).weakening && row.mainNetInflow > 0 && row.mainNetPct > 0);
  const score = row => Number(row.mainNetPct) * 2 + Math.min(5, Number(row.changePct) || 0)
    + assessSectorCapitalTrend(row).scoreAdjustment;
  const queues = ['industry','concept'].map(type => eligible.filter(row => row.boardType === type)
    .sort((a,b)=>score(b)-score(a) || b.mainNetInflow-a.mainNetInflow));
  const names = new Set(), selected = [];
  for (let index=0; index<Math.max(...queues.map(queue=>queue.length)) && selected.length<limit; index++) {
    for (const queue of queues) {
      const row=queue[index];
      if (!row || names.has(row.name) || selected.length>=limit) continue;
      names.add(row.name);
      selected.push(row);
    }
  }
...
function mergeSectorCapitalRows(rotation, capitalRows, marketQuotes = [], tradeDate = '') {
  tradeDate ||= (marketQuotes || []).map(row => row.tradeDate).filter(Boolean).sort().at(-1) || '';
  capitalRows = (capitalRows || []).filter(isSectorRotationTheme);
  const baseRows = rotation?.sectors || [];
  const baseByName = new Map(baseRows.map(item => [normalizeSectorCapitalName(item.name), item]));
  const directValues = (capitalRows || []).filter(item=>!item.capitalStale && (!tradeDate || item.capitalTradeDate === tradeDate)).map(item => finiteNumber(item.mainNetInflow)).filter(value => value !== null);
  const mergedByName = new Map();
  (capitalRows || []).forEach(row => {
    row = {...row, capitalStale:Boolean(row.capitalStale || tradeDate && row.capitalTradeDate !== tradeDate)};
    const name = normalizeSectorCapitalName(row.name);
    if (!name) return;
    const current = mergedByName.get(name);
    if (current && Math.abs(Number(current.mainNetInflow || 0)) >= Math.abs(Number(row.mainNetInflow || 0))) return;
    const base = baseByName.get(name) || {};
    const memberCodes = [...new Set((row.memberCodes || []).map(code => String(code || '').slice(-6)).filter(Boolean))];
    const memberStats = sectorQuoteStats(memberCodes, marketQuotes);
    const stats = memberStats || base;
    const mainNetInflow = finiteNumber(row.mainNetInflow);
    const mainNetPct = finiteNumber(row.mainNetPct);
    const direct = mainNetInflow !== null && !row.capitalStale;
    const breadthCount = stats.changes?.length ?? Number(stats.count || 0);
    const breadthAvailable = breadthCount > 0 && finiteNumber(stats.upRatio) !== null;
    const coverage = memberCodes.length ? Math.min(1, breadthCount / memberCodes.length) : breadthAvailable ? 1 : 0;
    // Shrink small or incomplete samples toward neutral; retain the observed ratio for display.
    const breadthScore = breadthAvailable ? 50 + (Number(stats.upRatio) * 100 - 50) * breadthCount / (breadthCount + 5) * coverage : 50;
    ...
    const changeScore = clampRecommendationScore(50 + changePct * 7);
    ...
    mergedByName.set(name, {
      ...
      name,
      changePct,
      averageChangePct:finiteNumber(stats.averageChangePct) ?? changePct,
      memberCodes,
      mainNetInflow,
      mainNetPct,
      capitalRank:Number(row.capitalRank) || null,
      capitalEstimated:mainNetInflow === null,
      capitalSource:row.source || stats.source || '全市场行业涨幅、上涨广度与成交活跃度',
      capitalFetchedAt:row.fetchedAt || '',
      rotationScore,
      capitalTrend,
      rotationPhase,
      participation,
      rotationState:row.capitalStale ? '资金缓存待核验' : rotationPhase.phase
    });
  });
  baseRows.forEach(row => {
    const name = normalizeSectorCapitalName(row.name);
    if (mergedByName.has(name)) return;
    mergedByName.set(name, {
      ...row,
      name,
      mainNetInflow:null,
      mainNetPct:null,
      capitalRank:null,
      capitalEstimated:true,
      capitalSource:row.source || '全市场行业涨幅、上涨广度与成交活跃度'
    });
  });
  const sectors = [...mergedByName.values()].sort((a, b) => Number(b.rotationScore || 0) - Number(a.rotationScore || 0)
    || Number(b.mainNetInflow || 0) - Number(a.mainNetInflow || 0) || Number(b.changePct || 0) - Number(a.changePct || 0));
  const directFunds = sectors.filter(item => !item.capitalEstimated && !item.capitalStale && finiteNumber(item.mainNetInflow) !== null)
    .sort((a, b) => Number(b.mainNetInflow) - Number(a.mainNetInflow));
  const estimatedFunds = sectors.filter(item => item.capitalEstimated || item.capitalStale)
    .sort((a, b) => Number(b.rotationScore || 0) - Number(a.rotationScore || 0));
  const weakSectors = [...sectors].sort((a, b) => {
    const aFlow = finiteNumber(a.mainNetInflow);
    const bFlow = finiteNumber(b.mainNetInflow);
    if (aFlow !== null || bFlow !== null) return Number(aFlow ?? Infinity) - Number(bFlow ?? Infinity);
    return Number(a.rotationScore || 0) - Number(b.rotationScore || 0);
  }).slice(0, 6);
  return { sectors, weakSectors, fundSectors:[...directFunds, ...estimatedFunds] };
}

function resolveStockRotationProfiles(stock, sectors) {
  const code = String(stock?.code || '').slice(-6);
  const industry = normalizeSectorCapitalName(resolveRecommendationIndustry(stock));
  return (sectors || []).filter(sector => {
    const members = sector.memberCodes || [];
    if (members.some(member => String(member || '').slice(-6) === code)) return true;
    const sectorName = normalizeSectorCapitalName(sector.name);
    return industry && industry !== '行业待确认' && sectorName === industry;
  }).sort((a, b) => Number(Boolean(a.capitalStale)) - Number(Boolean(b.capitalStale))
    || Number(a.capitalEstimated) - Number(b.capitalEstimated)
    || Number(b.rotationScore || 0) - Number(a.rotationScore || 0)
...
function selectRotationPriorityCandidates(scored, sectors, options = {}) {
  ...
  const eligibleSectors = (sectors || []).filter(item => !item.capitalStale && !assessSectorCapitalTrend(item).weakening && (item.capitalEstimated
    ...
    .sort((a, b) => Number(a.capitalEstimated) - Number(b.capitalEstimated)
      || Number(b.rotationScore || 0) - Number(a.rotationScore || 0)
      || Number(b.mainNetPct || 0) - Number(a.mainNetPct || 0)
      || Number(b.mainNetInflow || 0) - Number(a.mainNetInflow || 0));
  const selected = [];
  const codes = new Set();
  let coveredSectors = 0;
  const resolved = (scored || []).map(item => ({...item, rotationProfiles:item.rotationProfiles || resolveStockRotationProfiles(item, sectors)}));
  eligibleSectors.forEach(sector => {
    if (coveredSectors >= sectorLimit || selected.length >= limit) return;
    const rows = resolved
    ...
    rows.slice(0, perSector).forEach(item => {
      if (codes.has(item.code) || selected.length >= limit) return;
      codes.add(item.code);
      selected.push(item);
    });
  });
...
function momentumRecommendationDecision(item) {
  ...
  const score = passed ? clampRecommendationScore(Number(profile.rotationScore || 50) * .4
    ...
    + clampRecommendationScore(45 + Math.log10(Math.max(Number(item.amount || 0), 1)) * 5) * .2
    + (nearHigh >= .99 ? 85 : 70) * .15) : 0;
  return {
    passed,
    score,
    observationOnly:true,
...
function newsTimestamp(value) {
  ...
  let text = String(value || '').trim().replace(/^(\d{4}-\d{2}-\d{2})\s+/, '$1T');
...
function isMarketWideNews(item) {
...
function summarizeNews(news, sinceDate = '', options = {}) {
  ...
  let normalized = sortNewsNewestFirst(news || []).filter(item => {
    ...
    const recencyWeight = ageHours === null ? 0 : ageHours <= 6 ? 1 : ageHours <= 24 ? .9 : ageHours <= 72 ? .7 : ageHours <= 168 ? .4 : ageHours <= 720 ? .18 : 0;
    const relevance = !subject && !code ? 1
      : (subject && text.includes(subject)) || (code && text.includes(code)) ? 1 : .12;
    const clauses = text.split(/[。；;！!？?，,]/);
    const affirmed = term => clauses.some(clause => {
      const index = clause.indexOf(term);
      return index >= 0 && !/(?:否认|不存在|未涉及|未|无|不)[^，。；]{0,6}$/.test(clause.slice(0,index))
        && !/不实|传闻|取消|终止/.test(clause.slice(index + term.length));
    });
    ...
    const eventType = negative.some(([term]) => /退市|造假|立案|处罚|诉讼|减持/.test(term)) ? '公司风险'
      : positive.some(([term]) => /业绩|扭亏/.test(term)) ? '业绩'
        : positive.some(([term]) => /中标|合同|订单/.test(term)) ? '订单'
          : options.scope === 'market' ? '宏观市场' : '一般事件';
...
function weightedRecommendationScore(rawScore, qualityScore, factorScore) {
  const raw = finiteNumber(rawScore);
  const quality = finiteNumber(qualityScore);
  const factor = finiteNumber(factorScore);
  const parts = [raw === null ? null : [raw, .55], factor === null ? null : [factor, .3]];
  if (quality !== null) parts.push([quality, .15]);
  const available = parts.filter(Boolean);
  if (!available.length) return null;
  const weights = available.reduce((sum, [, weight]) => sum + weight, 0);
  return clampRecommendationScore(available.reduce((sum, [score, weight]) => sum + score * weight, 0) / weights);
}
...
function crossSectionPercentile(values, value) {
  const valid = values.map(finiteNumber).filter(item => item !== null).sort((a, b) => a - b);
  const numericValue = finiteNumber(value);
  if (valid.length < 2 || numericValue === null) return null;
  if (valid[0] === valid.at(-1)) return 50;
  const below = valid.filter(item => item < numericValue).length;
  const equal = valid.filter(item => item === numericValue).length;
  const averageRank = below + (Math.max(1, equal) + 1) / 2;
  return clampRecommendationScore(((averageRank - 1) / (valid.length - 1)) * 100);
}
...
function recommendationDataConfidence(item) {
  const fundFlow = item?.fundFlowPeriod;
  const stageCapitalAvailable = Boolean(fundFlow?.available && !fundFlow.estimated && !fundFlow.stale
    && (fundFlow.aggregateOnly || fundFlow.complete || Number(fundFlow.days) >= 5));
  const checks = [
    ['行业归属', resolveRecommendationIndustry(item) !== '行业待确认'],
    ['财务质量', finiteNumber(item?.qualityScore ?? item?.financialAnalysis?.quality?.score) !== null],
    ['多因子', Number(item?.factorAnalysis?.available) >= 4],
    ['财务序列', Number(item?.financialAnalysis?.available) >= 4 || Boolean(item?.financialAnalysis?.latestReport)],
    ['阶段资金', stageCapitalAvailable],
    ['消息时效', Boolean(item?.newsContext?.available || item?.newsContext?.items?.length)]
  ];
  const available = checks.filter(([, passed]) => passed).length;
  const score = clampRecommendationScore(available / checks.length * 100);
  return {
    score,
    label:score >= 80 ? '高' : score >= 50 ? '中等' : '低',
    available,
    total:checks.length,
...
function assessRecommendationHorizons(item) {
  const technical = finiteNumber(item?.technicalScore ?? item?.analysis?.score) ?? 50;
  const profile = item?.rotationProfiles?.find(row => !row.capitalStale) || item?.rotationProfiles?.[0];
  const sector = finiteNumber(profile?.rotationScore) ?? 50;
  const fundFlow = item?.fundFlowPeriod;
  const directCapital = Boolean(fundFlow?.available && !fundFlow.estimated && !fundFlow.stale
    && (fundFlow.aggregateOnly || fundFlow.complete || Number(fundFlow.days) >= 5));
  const capital = !directCapital ? 45
    : Number(fundFlow.mainNetInflow) > 0 && Number(fundFlow.netRatio) > 0 ? 82 : 28;
  const news = item?.newsLabel === '消息确认' ? 78 : item?.newsLabel === '消息谨慎' ? 25 : 50;
  const latest = finiteNumber(item?.price ?? item?.analysis?.latestPrice);
  const ma20 = finiteNumber(item?.analysis?.ma20), ma30 = finiteNumber(item?.analysis?.ma30), ma60 = finiteNumber(item?.analysis?.ma60);
  const trend = latest !== null && ma20 !== null && ma30 !== null && ma60 !== null
    ? latest >= ma20 && ma20 >= ma30 && ma30 >= ma60 ? 85
      : latest >= ma30 && ma30 >= ma60 ? 68
        : latest < ma30 && ma30 < ma60 ? 25 : 50
    : 45;
  const financial = finiteNumber(item?.financialAnalysis?.score ?? item?.financialAnalysis?.quality?.score ?? item?.qualityScore);
  const canslim = finiteNumber(item?.canslim?.score);
  const volumeRatio = finiteNumber(item?.analysis?.volumeRatio ?? item?.volumeRatio);
  const shortScore = clampRecommendationScore(technical * .5 + sector * .2 + capital * .2 + news * .1
    - (isExplosiveVolume(volumeRatio) ? 20 : 0));
  const swingScore = clampRecommendationScore(technical * .4 + sector * .2 + capital * .2 + trend * .2);
  // Financial quality can be embedded in CANSLIM; count the fundamental block once.
  const longParts = [financial, canslim, trend].filter(value => value !== null);
  const longScore = longParts.length >= 2 ? clampRecommendationScore(
    (financial ?? 50) * .4 + (canslim ?? 50) * .35 + trend * .25) : 0;
  const blocked = Boolean(item?.entryAssessment?.contextRisks?.length)
    || ['破位', '结构偏弱', '公司风险'].includes(item?.entryAssessment?.status);
  const profiles = [
    {
      label:'短线', days:'1-5个交易日', score:shortScore,
      qualified:!blocked && shortScore >= 70 && Boolean(item?.momentumDecision?.passed || ['突破确认','接近突破'].includes(item?.signal)),
      basis:`技术${technical}、板块${sector}、资金${capital}、消息${news}`
    },
    {
      label:'波段', days:'5-20个交易日', score:swingScore,
      qualified:!blocked && swingScore >= 65 && trend >= 50 && !analyzeCapitalWindows(item?.fundFlowPeriod, item?.analysis?.tradeDate).weakening,
      basis:`技术${technical}、板块${sector}、阶段资金${capital}、趋势${trend}`
    },
    {
      label:'中长线', days:'20-120个交易日', score:longScore,
      qualified:!blocked && longParts.length >= 3 && longScore >= 65 && trend >= 50,
      basis:longParts.length >= 2 ? `基本面、CANSLIM与中期趋势共${longParts.length}项可核验（财务质量只计一次）` : '长期财务与趋势数据不足'
    }
  ];
  const qualified = profiles.filter(row => row.qualified);
  const primary = item?.momentumDecision?.passed && profiles[0].qualified
    ? profiles[0]
    : qualified.length ? [...qualified].sort((a, b) => b.score - a.score)[0] : null;
  return { primary:primary?.label || '周期待确认', days:primary?.days || '', score:primary?.score ?? null, profiles };
}

function buildStrategyScoreCard(item, snapshot = {}) {
  const technical = finiteNumber(item?.technicalScore ?? item?.analysis?.score);
  const recommendation = finiteNumber(item?.signalScore ?? item?.score);
  const canslim = finiteNumber(item?.canslim?.score);
  const factor = finiteNumber(item?.factorAnalysis?.score);
  const confidence = finiteNumber(item?.dataConfidence?.score);
  return {
    schemaVersion:1,
    snapshotId:snapshot.snapshotId || item?.recommendationContext?.snapshotId || '',
    tradeDate:snapshot.tradeDate || item?.analysis?.tradeDate || '',
    observedAt:snapshot.observedAt || item?.recommendationContext?.evaluatedAt || '',
    observationPhase:snapshot.observationPhase || item?.analysis?.observationPhase || null,
    ...
    factor,
    confidence,
    labels:{technical:'技术评分',recommendation:'推荐评分',canslim:'CANSLIM评分',factor:'多因子评分',confidence:'数据可信度'},
    note:'各分数含义不同；推荐列表、详情和收藏应读取同一快照字段，刷新后不得覆盖收藏时快照。'
  };
}

function filterResolvedRecommendations(items) {
  const result = { items:[], rejected:[] };
  for (const item of items || []) {
    const industry = resolveRecommendationIndustry(item);
    if (industry === '行业待确认') result.rejected.push({code:String(item?.code || ''), reason:'industry-unresolved'});
    else result.items.push({...item, industry});
  }
  return result;
}
...
function attachStrategyPlatform(item, context = {}) {
  const analysis = item.analysis || {};
  const leadership = finiteNumber(item.factorAnalysis?.factors?.leadership?.score);
  const factor = {
    code:item.code,
    ...
    rps120:leadership,
    rps250:leadership,
    rpsScope:'current-candidate-sample',
    dataCoverage:{history:Number(analysis.historyCoverage || 0)}
  };
  ...
  evaluations.push({
    id:'legacy-v18',
    matched:true,
    score:clampRecommendationScore(item.signalScore ?? item.score),
    stage:item.signal || item.entryAssessment?.status || '待确认',
    horizon:item.holdingPeriod || item.holdingProfile?.primary || '周期待确认',
    reasons:[item.reason || item.entryAssessment?.summary || '现有策略通过'],
    evidenceIds:['legacy-analysis']
  });
...
function buildRecommendationFactorContext(items) {
  ...
  const sectorProfiles = new Map([...sectors.entries()].map(([key, peers]) => {
    ...
    const averageChangePct = changes.length ? average(changes) : 0;
    const upRatio = changes.length ? changes.filter(value => value > 0).length / changes.length : .5;
    const amountPerStock = amounts.length ? amounts.reduce((sum, value) => sum + value, 0) / amounts.length : 0;
    const liquidityScore = percentileScore(rows.map(item => item.amount), amountPerStock);
    const score = clampRecommendationScore(45 + averageChangePct * 7 + (upRatio - .5) * 40 + (liquidityScore - 50) * .15);
    const label = score >= 72 ? '资金升温板块' : score >= 58 ? '轮动活跃板块' : score < 42 ? '资金退潮板块' : '轮动中性板块';
    return [key, {
      name, count: peers.length, averageChangePct, upRatio,
      amount: amounts.reduce((sum, value) => sum + value, 0), amountPerStock, liquidityScore, score, label,
      rotationState: score >= 72 ? '资金升温' : score >= 58 ? '轮动活跃' : score < 42 ? '资金退潮' : '轮动中性',
      changes, amounts
    }];
  }));
...
function buildDetailFactorAnalysis(analysis = {}, fundFlow = null, sector = null) {
  const factors = [];
  const add = (key, score, evidence) => {
    const value = finiteNumber(score);
    if (value !== null) factors.push({key, score:clampRecommendationScore(value), evidence});
  };
  add('technical', analysis.score, `技术评分${analysis.score}`);
  if (finiteNumber(analysis.return20) !== null || finiteNumber(analysis.return60) !== null) {
    add('momentum', 50 + Number(analysis.return20 || 0) * 1.2 + Number(analysis.return60 || 0) * .5,
      `近20日${signedPercent(analysis.return20)}，近60日${signedPercent(analysis.return60)}`);
  }
  if (finiteNumber(analysis.latestPrice) !== null && finiteNumber(analysis.ma20) !== null && finiteNumber(analysis.ma30) !== null) {
    add('trend', analysis.latestPrice >= analysis.ma20 && analysis.ma20 >= analysis.ma30 ? 82 : analysis.latestPrice >= analysis.ma30 ? 58 : 28,
      `现价${analysis.latestPrice}，MA20 ${analysis.ma20}，MA30 ${analysis.ma30}`);
  }
  if (finiteNumber(analysis.volumeRatio) !== null) {
    add('volume', isControlledVolumeExpansion(analysis.volumeRatio) ? 82 : isExplosiveVolume(analysis.volumeRatio) ? 25 : 52,
      `量比${analysis.volumeRatio}`);
  }
  if (fundFlow?.available && !fundFlow.estimated && !fundFlow.stale && finiteNumber(fundFlow.mainNetInflow) !== null) {
    add('capital', Number(fundFlow.mainNetInflow) > 0 ? 72 + Math.min(18, Math.max(0, Number(fundFlow.netRatio || 0) * 3)) : 28,
      `阶段主力净额${fundFlow.mainNetInflow}`);
  }
  if (finiteNumber(sector?.rotationScore) !== null) add('sectorRotation', sector.rotationScore, `板块轮动评分${sector.rotationScore}`);
  return {
    score:factors.length ? clampRecommendationScore(average(factors.map(item => item.score))) : null,
    available:factors.length,
    total:6,
    missing:['technical','momentum','trend','volume','capital','sectorRotation'].filter(key => !factors.some(item => item.key === key)),
    factors,
    scope:'当前个股快照多因子；缺失项不计分'
  };
}
...
function mergeFundFlowSnapshot(history, current, tradeDate) {
  const merged = {...history, current};
  if (history?.available && !history.estimated && !history.stale && history.endDate === tradeDate && history.rows?.length >= 10) return merged;
  const windows = analyzeCapitalWindows(merged, tradeDate);
  if (!windows.aggregateOnly) return merged;
  const days = windows.net10 !== null ? 10 : 5;
  return {...merged, available:true, estimated:false, aggregateOnly:true, days,
    mainNetInflow:days === 10 ? windows.net10 : windows.net5,
    netRatio:finiteNumber(days === 10 ? current.mainNetPct10 : current.mainNetPct5),
    mainInflow:null, mainOutflow:null, positiveDays:null, negativeDays:null,
    grossFlowsAvailable:false, rows:history?.estimated ? [] : history?.rows || [], endDate:tradeDate,
    source:`${current.source || '东方财富资金'}（真实多日累计）`, summary:windows.summary};
}

function fundFlowPeriodEvidence(flow) {
  if (flow?.aggregateOnly) return flow.summary;
  if (!flow?.available) return '阶段主力资金数据不可用';
  if (flow.estimated) {
    const direction = flow.mainNetInflow >= 0 ? '偏流入' : '偏流出';
    return `近${flow.days}日量价资金${direction}${formatCapitalAmount(flow.mainNetInflow)}，资金强度${Number(flow.netRatio || 0) >= 0 ? '+' : ''}${Number(flow.netRatio || 0).toFixed(2)}%，正向${flow.positiveDays}日（非Level-2主力口径）`;
  }
...
function analyzeCapitalWindows(flow, tradeDate = '') {
  ...
  if (currentValid && (finiteNumber(current.mainNet5) !== null || finiteNumber(current.mainNet10) !== null)) {
    const trend = assessSectorCapitalTrend(current);
    return {available:true, stale:false, endDate:tradeDate, latestNet:current.mainNetInflow, latestRatio:current.mainNetPct,
      net3:finiteNumber(current.mainNet3), net5:finiteNumber(current.mainNet5), net10:finiteNumber(current.mainNet10),
      weakening:trend.weakening, confirmed:trend.confirmed, summary:trend.summary, aggregateOnly:true};
  }
  const byDate = new Map();
  for (const row of flow?.rows || []) {
    const date = dateOnly(row?.date);
    if (date && finiteNumber(row.mainNetInflow) !== null && (!tradeDate || date <= tradeDate)) byDate.set(date,row);
  }
  const rows = [...byDate].sort(([a],[b])=>a.localeCompare(b)).map(([date,row])=>({...row,date})).slice(-10);
  ...
  return {
    available:true, stale, endDate, latestNet, latestRatio, net3, net10, recentOutDays, weakening, confirmed,
    summary:`资金截至${endDate}，最近1日净额${signedNet(latestNet)}、3日${signedNet(net3)}、10日${signedNet(net10)}；${stale ? '与行情日期不同步，暂停资金确认' : weakening ? '近期资金转弱，累计净流入不能覆盖短期流出风险' : confirmed ? '短期资金方向一致' : '资金方向尚未同步'}`
  };
}
...
function combineConsolidationBreakout(technical, flow) {
  ...
  const flowAvailable = Boolean(windows.confirmed || flow?.available && !flow.estimated && flow.days && !windows.stale);
  const persistentOutflow = windows.weakening || flowAvailable && (flow.mainNetInflow < 0
    && flow.netRatio <= -3 && finiteNumber(flow.positiveDays) !== null && flow.positiveDays <= Math.floor(flow.days * .4));
  const flowAdjustment = persistentOutflow ? -18 : windows.confirmed ? 10 : !flowAvailable ? 0
...
function applyIndividualCapitalAssessment(analysis, flow) {
  ...
  const capitalWindows = analyzeCapitalWindows(flow, analysis.tradeDate);
  const available = Boolean(flow?.available && flow.days && !capitalWindows.stale);
  const directFlow = available && !flow.estimated;
  const strongFlow = capitalWindows.confirmed || directFlow && flow.mainNetInflow > 0
    && !capitalWindows.available && flow.positiveDays >= Math.ceil(flow.days * .6);
  const persistentOutflow = capitalWindows.weakening || available && (flow.mainNetInflow < 0
    && flow.netRatio <= (flow.estimated ? -6 : -3) && finiteNumber(flow.positiveDays) !== null && flow.positiveDays <= Math.floor(flow.days * .4));
  ...
  const adjustment = capitalWindows.weakening ? -8 : capitalWindows.confirmed ? 6 : flow?.estimated ? Math.min(0, Math.round(rawAdjustment * .5)) : rawAdjustment;
  const status = capitalWindows.weakening ? '资金偏弱' : capitalWindows.confirmed ? '真实多窗口资金同向' : !available ? '资金数据不足'
  ...
  const breakoutPotential = combineConsolidationBreakout({...analysis.consolidationBreakout, tradeDate:analysis.tradeDate}, flow);
  ...
  if (entryAssessment && (persistentOutflow || breakoutWeak)) {
    ...
    const capitalEvidence = flow.aggregateOnly ? fundFlowPeriodEvidence(flow)
      : `近${flow.days}日阶段主力净流入${formatCapitalAmount(flow.mainNetInflow)}，净流入${flow.positiveDays}日`;
    entryAssessment = {
      ...entryAssessment,
      allowed: true,
  ...
  } else if (entryAssessment?.allowed && directFlow && flow.mainNetInflow > 0) {
    entryAssessment = { ...entryAssessment, summary: `${entryAssessment.summary} 阶段主力净流入${formatCapitalAmount(flow.mainNetInflow)}，资金方向同步。` };
  } else if (entryAssessment?.allowed && flow?.estimated && flow.mainNetInflow > 0) {
    entryAssessment = { ...entryAssessment, summary: `${entryAssessment.summary} 日线量价资金偏流入，但该数据不是Level-2主力口径，只作辅助确认。` };
  ...
  return {
    ...
    tradePlan: analysis.tradePlan ? {
      ...analysis.tradePlan,
      enabled: entryAllowed ? true : persistentOutflow || breakoutWeak || entryBlockedByFunds || capitalWindows.stale ? false : analysis.tradePlan.enabled
    } : analysis.tradePlan,
    entryAssessment,
    breakoutPotential,
...
function globalMarketRiskAdjustment(overseas, subject) {
  if (!overseas?.available || overseas.stale || overseas.severity === 'low') return 0;
...
function assessMarketEnvironment(overview) {
  ...
  const up = Number(overview?.breadth?.up || 0), down = Number(overview?.breadth?.down || 0);
  const weak = indexAverage !== null && indexAverage <= -1 || down > 0 && down > up * 1.3;
  const strong = !weak && (indexAverage !== null && indexAverage >= .4 || up > 0 && up > down * 1.3);
  return {weak,strong,up,down,label:!changes.length && !up && !down ? '数据不足' : weak ? '偏弱' : strong ? '偏强' : '分化'};
}

function classifyMarketRegime(overview = {}, previous = null) {
  const up = Number(overview?.breadth?.up || 0), down = Number(overview?.breadth?.down || 0), flat = Number(overview?.breadth?.flat || 0);
  const active = up + down;
  const upRatio = active ? up / active : null;
  ...
  const broadStrong = upRatio !== null && upRatio >= .65 && (indexAverage === null || indexAverage >= 0);
  const volumeExpanded = turnoverChangePct !== null && turnoverChangePct >= 10;
...
function applyEntryContextAssessment(analysis, { newsContext = null, riskProfile = null, marketOverview = null, marketNewsContext = null, subject = null } = {}) {
  ...
  const {weak:marketWeak,label:marketLabel} = assessMarketEnvironment(marketOverview);
  ...
  const usGrowthRisk = hasOverseas && !overseas.stale && overseas.severity === 'high' && isUSGrowthSensitive(subject);
  const profiles = subject?.rotationProfiles || resolveStockRotationProfiles(subject, marketOverview?.sectors);
  ...
  const sectorRotationSummary = relatedSector
    ...
    : '';
  if (sectorRotationSummary) evidence.push(sectorRotationSummary);
  if (relatedSector?.capitalStale) evidence.push('板块资金为过期缓存，不参与当前资金确认');
  ...
  } else if (outsideLocalizedMainline && !technicalRisk) {
...
function evaluateRecommendationFactors(item, context, newsContext, financialAnalysis = null) {
  const directSector = item?.rotationProfiles?.[0];
  const industryPeers = context?.sectorProfiles?.get(recommendationIndustryGroupKey(item));
  const sector = directSector ? {
    ...directSector,
    score:directSector.capitalStale ? 50 : Number(directSector.rotationScore || 50),
    label:directSector.rotationState || '轮动中性',
    changes:directSector.changes || (directSector.name === industryPeers?.name ? industryPeers.changes : []),
    amounts:directSector.amounts || (directSector.name === industryPeers?.name ? industryPeers.amounts : [])
  } : industryPeers;
  const factors = [];
  ...
  const directFlowScore = flow?.available
    ? clampRecommendationScore(50 + Number(flow.netRatio || 0) * 5
      + (finiteNumber(flow.positiveDays) !== null && flow.days > 0 ? (flow.positiveDays / flow.days - .5) * 45 : 0)) : 0;
  const flowScore = flow?.estimated ? clampRecommendationScore(50 + (directFlowScore - 50) * .5) : directFlowScore;
  ...
  const peerComparisonAvailable = sector?.changes?.length > 1 && sector?.amounts?.length > 1;
  const leadershipScore = sector
    ? average([relativeStrength, peerComparisonAvailable ? percentileScore(sector.changes, item.changePct) : 50,
      peerComparisonAvailable ? percentileScore(sector.amounts, item.amount) : 50, sector.score]) : 0;
  add('leadership', '板块领先', leadershipScore,
    sector ? `${sector.name}${sector.label}，${sector.capitalEstimated === false ? `主力净${Number(sector.mainNetInflow) >= 0 ? '流入' : '流出'}${formatCapitalAmount(Math.abs(Number(sector.mainNetInflow || 0)))}、占比${signedPercent(sector.mainNetPct)}` : `上涨占比${(Number(sector.upRatio || 0) * 100).toFixed(0)}%`}，近60日相对强度分位${relativeStrength}，${peerComparisonAvailable ? `与${sector.changes.length}只同行比较涨幅和成交活跃度` : '同行样本未取得，相对领先度按中性处理'}` : '行业分类不可用', Boolean(sector));
  ...
  const newsAvailable = Boolean(newsContext?.available ?? newsContext?.items?.length);
  ...
  return {
    ...
    sectorProfile: sector ? {
      ...
      upRatio: sector.upRatio, amount: sector.amount, amountPerStock:sector.amountPerStock,
      liquidityScore:sector.liquidityScore, score: sector.score, label: sector.label, rotationState:sector.rotationState
    } : null
  };
}
...
function assessRecommendationNewsConfirmation(item, newsContext = {}) {
  ...
  const capitalWindows = analyzeCapitalWindows(flow, item?.analysis?.tradeDate);
  const capitalConfirmed = Boolean(flow?.available && !flow.estimated && !capitalWindows.stale
    && (!capitalWindows.available || capitalWindows.confirmed) && flow.days > 0 && flow.mainNetInflow > 0
    && Number(flow.netRatio) > 0 && (capitalWindows.aggregateOnly ? capitalWindows.confirmed : flow.positiveDays >= Math.ceil(flow.days * .6)));
...
function mergeRecommendationOutcomeQuotes(rows, quotes) {
  const quoteByCode = new Map((Array.isArray(quotes) ? quotes : [])
    ...
    .map(quote => [String(quote.code), quote]));
  return (Array.isArray(rows) ? rows : []).map(row => {
    const quote = quoteByCode.get(String(row?.code || ''));
    return quote
      ? { ...row, price:Number(quote.price), quoteMatched:true, quoteFetchedAt:quote.fetchedAt || '' }
      : { ...row, quoteMatched:false };
  });
}
...
function recommendationOutcomeStats(rows) {
  ...
  const marketExcessReturns = rows.map(row => row.marketExcessReturnPct).filter(Number.isFinite).sort((a, b) => a - b);
  const industryExcessReturns = rows.map(row => row.industryExcessReturnPct).filter(Number.isFinite).sort((a, b) => a - b);
  ...
  const benchmarkStats = values => values.length ? {
    average:roundMetric(average(values), 2),
    median:roundMetric(values.length % 2 ? values[Math.floor(values.length / 2)] : (values[values.length / 2 - 1] + values[values.length / 2]) / 2, 2),
    outperformRate:roundMetric(values.filter(value => value > 0).length / values.length * 100, 1)
  } : null;
  const market = benchmarkStats(marketExcessReturns);
  const industry = benchmarkStats(industryExcessReturns);
  return {
    count: rows.length,
    ...
    averagePeerRelativeReturn: roundMetric(average(excessReturns), 2),
    medianPeerRelativeReturn: roundMetric(excessMedian, 2),
    peerOutperformRate: roundMetric(rows.filter(row => row.excessReturnPct > 0).length / rows.length * 100, 1),
    marketBenchmarkCount: marketExcessReturns.length,
    averageMarketExcessReturn: market?.average ?? null,
    medianMarketExcessReturn: market?.median ?? null,
    marketOutperformRate: market?.outperformRate ?? null,
    industryBenchmarkCount: industryExcessReturns.length,
    averageIndustryExcessReturn: industry?.average ?? null,
    medianIndustryExcessReturn: industry?.median ?? null,
    industryOutperformRate: industry?.outperformRate ?? null
  };
}

function mergeRecommendationOutcomeBenchmarks(rows, indices, sectors = []) {
  const current = new Map((indices || []).filter(index => Number(index?.price) > 0)
    .map(index => [String(index.code || ''), Number(index.price)]));
  const currentSectors = new Map((sectors || []).filter(sector => Number(sector?.price) > 0)
    .map(sector => [String(sector.code || ''), Number(sector.price)]));
  return (rows || []).map(row => {
    const returns = (row?.benchmarkIndices || []).map(index => {
      const base = Number(index?.price);
      const price = current.get(String(index?.code || ''));
      return base > 0 && price > 0 ? price / base - 1 : null;
    }).filter(Number.isFinite);
    const industryBase = Number(row?.industryBenchmark?.price);
    const industryPrice = currentSectors.get(String(row?.industryBenchmark?.code || ''));
    const benchmark = returns.length ? {
      ...row,
      benchmarkBasePrice:100,
      benchmarkPrice:100 * (1 + average(returns)),
      benchmarkCount:returns.length
    } : row;
    return industryBase > 0 && industryPrice > 0 ? {
      ...benchmark,
      industryBenchmarkBasePrice:industryBase,
      industryBenchmarkPrice:industryPrice
    } : benchmark;
  });
}
...
function summarizeRecommendationOutcomes(rows, { now = Date.now(), minimumAgeDays = 1, recentDays = 4, minimumRecentSamples = 20, requireFreshQuote = false } = {}) {
  ...
  for (const row of Array.isArray(rows) ? rows : []) {
    ...
    if (requireFreshQuote && row?.quoteMatched !== true) {
      staleQuoteCount++;
      continue;
    }
    ...
    const identity = row.code ? `${row.code}|${addedAt}|${base}` : '';
    ...
    const returnPct = (price / base - 1) * 100;
    const benchmarkBase = Number(row?.benchmarkBasePrice);
    const benchmarkPrice = Number(row?.benchmarkPrice);
    const industryBenchmarkBase = Number(row?.industryBenchmarkBasePrice);
    const industryBenchmarkPrice = Number(row?.industryBenchmarkPrice);
    const marketReturnPct = benchmarkBase > 0 && benchmarkPrice > 0 ? (benchmarkPrice / benchmarkBase - 1) * 100 : null;
    const industryReturnPct = industryBenchmarkBase > 0 && industryBenchmarkPrice > 0
      ? (industryBenchmarkPrice / industryBenchmarkBase - 1) * 100 : null;
    eligible.push({
      label,
      addedAt,
      signal,
      family: signal ? recommendationSignalFamily(signal) : '',
      ...
      holdingPeriod:String(row?.holdingPeriod || row?.favoriteEntrySnapshot?.holdingPeriod || '周期未标记'),
      returnPct,
      marketReturnPct,
      industryReturnPct,
      marketExcessReturnPct:marketReturnPct === null ? null : returnPct - marketReturnPct,
      industryExcessReturnPct:industryReturnPct === null ? null : returnPct - industryReturnPct
    });
  }
  ...
  const byHorizon = {};
  const byRecentHorizon = {};
  for (const family of [...new Set(samples.map(row => row.family).filter(Boolean))]) {
  ...
  for (const family of [...new Set(recentSamples.map(row => row.family).filter(Boolean))]) {
  ...
  for (const horizon of [...new Set(samples.map(row => row.holdingPeriod).filter(Boolean))]) {
    byHorizon[horizon] = recommendationOutcomeStats(samples.filter(row => row.holdingPeriod === horizon));
  }
  for (const horizon of [...new Set(recentSamples.map(row => row.holdingPeriod).filter(Boolean))]) {
    byRecentHorizon[horizon] = recommendationOutcomeStats(recentSamples.filter(row => row.holdingPeriod === horizon));
  }
  ...
  profile.signature = JSON.stringify({
    ...
    recentOverall:profile.recentOverall,
    byFamily, bySignal, byScoreBand, byTechnicalScoreBand, byHorizon,
...
function recommendationOutcomeMetadata(profile) {
  if (!profile) return null;
  return {
    sampleSize: profile.sampleSize,
    attributedCount: profile.attributedCount,
    ...
    bySignal: profile.bySignal,
    byScoreBand: profile.byScoreBand,
    byTechnicalScoreBand: profile.byTechnicalScoreBand,
    byHorizon: profile.byHorizon,
    byRecentSignal: profile.byRecentSignal,
    byRecentTechnicalScoreBand: profile.byRecentTechnicalScoreBand
  };
}

function outcomeComparison(stats, minimumCount = 1) {
  const benchmarkCoverage = prefix => Number(stats[`${prefix}BenchmarkCount`]) >= minimumCount
    && Number(stats[`${prefix}BenchmarkCount`]) >= Number(stats.count) * .6;
  return benchmarkCoverage('industry')
    ? {label:'行业基准', rateLabel:'行业基准', average:Number(stats.averageIndustryExcessReturn), median:Number(stats.medianIndustryExcessReturn), rate:Number(stats.industryOutperformRate)}
    : benchmarkCoverage('market')
      ? {label:'大盘基准', rateLabel:'大盘基准', average:Number(stats.averageMarketExcessReturn), median:Number(stats.medianMarketExcessReturn), rate:Number(stats.marketOutperformRate)}
      : {label:'同批次中位数', rateLabel:'同批次', average:Number(stats.averageExcessReturn), median:Number(stats.medianExcessReturn), rate:Number(stats.outperformRate)};
}

function outcomeStatsAdjustment(stats, minimumCount) {
  if (!stats || Number(stats.count) < minimumCount) return 0;
  if (finiteNumber(stats.cohortCount) !== null && stats.cohortCount < 2) return 0;
  const relative = outcomeComparison(stats, minimumCount);
  if (Number(stats.averageReturn) <= -4 && Number(stats.winRate) < 30) return -6;
  if (Number(stats.averageReturn) <= -2 && Number(stats.winRate) < 35) return -4;
  if (Number(stats.averageReturn) <= -1 && Number(stats.winRate) < 40) return -2;
  if (relative.rate < 35 && relative.median < -.5) return -6;
  if (relative.rate < 45 && relative.median < -.5) return -4;
  if (relative.rate < 50 && relative.average <= -1 && relative.median < -.5) return -4;
  ...
  if (relative.rate >= 65 && relative.median >= .5) return 5;
  if (relative.rate >= 55 && relative.average >= 1) return 2;
  return 0;
}
...
function calibrateRecommendationWithOutcomes(item, profile) {
  ...
  const familyComparison = familyStats ? outcomeComparison(familyStats, recent ? 6 : 12) : null;
  const technicalComparison = technicalStats ? outcomeComparison(technicalStats, technicalStats === recentTechnicalStats ? 6 : 8) : null;
  const longTermTechnicalComparison = longTermTechnicalStats ? outcomeComparison(longTermTechnicalStats, 8) : null;
  const summary = familyStats
    ? `${label}${recent ? '近期' : ''}同类样本${familyStats.count}条，平均累计${signedPercent(familyStats.averageReturn)}，${familyComparison.label}相对${familyComparison.average >= 0 ? '领先' : '落后'}${Math.abs(familyComparison.average).toFixed(2)}%，${familyComparison.rateLabel}领先率${familyComparison.rate.toFixed(1)}%`
    : '本地同类推荐样本不足，暂不调整';
  const technicalSummary = technicalStats
    ? `；${technicalRecent ? '近期' : ''}技术分${technicalScoreBand}样本${technicalStats.count}条，平均累计${signedPercent(technicalStats.averageReturn)}，胜率${Number(technicalStats.winRate).toFixed(1)}%，${technicalComparison.label}相对${technicalComparison.average >= 0 ? '领先' : '落后'}${Math.abs(technicalComparison.average).toFixed(2)}%`
    : '';
  const longTermTechnicalSummary = technicalRecent && Number(longTermTechnicalStats?.count) > Number(technicalStats?.count)
    ? `；长期技术分${technicalScoreBand}样本${longTermTechnicalStats.count}条，平均累计${signedPercent(longTermTechnicalStats.averageReturn)}，胜率${Number(longTermTechnicalStats.winRate).toFixed(1)}%，${longTermTechnicalComparison.label}相对${longTermTechnicalComparison.average >= 0 ? '领先' : '落后'}${Math.abs(longTermTechnicalComparison.average).toFixed(2)}%`
    : '';
  return {
    family,
    scoreBand,
...
function recommendationGateDecision(item) {
  ...
  return { passed: false, reason: 'signal-score' };
}

function recommendationPassesOutcomeGate(item) {
  return recommendationGateDecision(item).passed;
}
...
function recommendationPassesWatchGate(item, postRisk = false) {
  ...
  if (distance < -1.5 || distance > 4) return false;
  if (isExplosiveVolume(item?.analysis?.volumeRatio)) return false;
  if (item?.newsLabel === '消息谨慎') return false;
  if (['破位', '爆量观察', '结构偏弱', '不宜追高', '公司风险', '消息风险待确认', '大盘消息风险待确认', '美股科技风险待确认', '板块退潮，等待确认'].includes(item?.entryAssessment?.status)) return false;
  if (item?.fundFlowPeriod?.available && item.fundFlowPeriod.netRatio <= (item.fundFlowPeriod.estimated ? -6 : -3) && finiteNumber(item.fundFlowPeriod.positiveDays) !== null && item.fundFlowPeriod.positiveDays <= 4) return false;
  if (item?.financialAnalysis?.hardRisks?.length >= 2 && item.financialAnalysis.score < 45) return false;
  if (item?.canslim?.available >= 5 && item.canslim.score < 50) return false;
  return true;
}
...
function finalizeRecommendationDisplay(strictItems, watchItems) {
  ...
  const watches = (watchItems || []).filter(item => {
    ...
    .map(item => ({
      ...
      entryAssessment: {
        ...(item.entryAssessment || {}),
        allowed: false,
        status: item.signal === '已反弹' ? '趋势延续观察，等待确认' : '高分观察，等待形态确认',
        tone: 'warning',
        summary: `${item.entryAssessment?.summary || '当前技术质量较高，但形态尚未确认。'} 本项仅作为观察候选，不构成入场信号。`
      },
      reason: `${item.signal === '已反弹' ? '趋势延续观察' : '横盘观察'}；${item.reason || '技术和质量评分较高，但尚未形成明确突破形态。'} 本项仅作为观察候选，不构成入场信号。`
    }));
  return [...strict, ...watches];
}
...
function applyOutcomeFeedbackAssessment(analysis, profile, signalOverride = '') {
    ...
    : analysis.entryAssessment?.setupType === 'sideways-washout' ? '震荡洗盘'
      : analysis.reboundSignal === '已反弹' ? '已反弹'
        : analysis.reboundSignal === '底部待反弹' ? '底部待反弹'
          : analysis.consolidationBreakout?.status || '待突破');
  ...
  const upperTechnicalHistory = !currentStrong && outcome.technicalScoreBand === '65-74'
    && outcome.technicalRecent && Number(outcome.technicalStats?.count) >= 6
    && (Number(outcome.technicalStats?.averageReturn) < 0 || Number(outcome.technicalStats?.winRate) < 40);
  ...
  if (!severeTechnicalHistory && !middleTechnicalHistory && !upperTechnicalHistory && !feedbackGate) {
  ...
  const entryAssessment = {
    ...
    status: severeTechnicalHistory && !preserveCurrentStatus ? '历史同类技术分偏弱，不建议入场'
      ...
      : upperTechnicalHistory && !preserveCurrentStatus ? '近期同技术分胜率偏低，等待确认'
  ...
  return {
    ...
    verdict: severeTechnicalHistory ? '暂不适合'
      : (middleTechnicalHistory || upperTechnicalHistory) && analysis.verdict === '可关注' ? '等待确认'
...
function canReuseMarketRecommendations(cachedOverview, { feedbackSignature = '', hasDirectSectorCapital = null, marketRegimeSignature = null, now = Date.now(), maxAgeMs = 10 * 60 * 1000 } = {}) {
  const savedAt = Date.parse(cachedOverview?.recommendationsFetchedAt || cachedOverview?.fetchedAt || '');
  const cachedHasDirectSectorCapital = Number(cachedOverview?.recommendationCoverage?.directSectorCapital || 0) > 0;
  return Boolean(cachedOverview?.recommendations?.length
    && !cachedOverview.cachedFallback && !cachedOverview.recommendationFallback?.active
    && Array.isArray(cachedOverview.momentumRecommendations)
    && cachedOverview.recommendationCoverage?.modelVersion === RECOMMENDATION_MODEL_VERSION
    && cachedOverview.recommendationCoverage?.outcomeFeedback?.signature === feedbackSignature
    && (hasDirectSectorCapital === null || cachedHasDirectSectorCapital === hasDirectSectorCapital)
    && (marketRegimeSignature === null || cachedOverview.recommendationCoverage?.marketRegimeSignature === marketRegimeSignature)
    && Number.isFinite(savedAt)
    && now >= savedAt
    && now - savedAt < maxAgeMs);
}

function deactivateCachedRecommendation(item) {
  const entryAssessment = {allowed:false, status:'数据待补充', tone:'neutral',
    summary:'当前为旧缓存推荐，需刷新走势、资金、消息面和大盘数据后重新评估。', evidence:[]};
  return {...item, stale:true, newsLabel:'消息待核验', entryAssessment,
    ...(item.tradePlan ? {tradePlan:{...item.tradePlan,enabled:false}} : {}),
    reason:`缓存仅供回看；综合入场：${entryAssessment.status}，${entryAssessment.summary}`};
}
...
function restoreCachedMarketRecommendations(result, cachedOverview, marketQuotes = [], { reuse = false } = {}) {
  ...
  result.recommendations = eligibleRecommendations.map(item => {
    ...
    const snapshot = reuse && item.entryAssessment ? item : deactivateCachedRecommendation(item);
    const entryAssessment = snapshot.entryAssessment;
    const reason = reuse && /综合入场/.test(item.reason || '')
    ...
    return {
      ...
      entryAssessment,
      reason:reuse ? reason : snapshot.reason
    };
  }).filter(item => item.industry !== '行业待确认').map(attachRecommendationMetadata);
  const cachedModelCurrent = cachedOverview.recommendationCoverage?.modelVersion === RECOMMENDATION_MODEL_VERSION;
  if (!(result.momentumRecommendations || []).length) result.momentumRecommendations = (reuse && cachedModelCurrent ? cachedOverview.momentumRecommendations || [] : []).filter(item => item.momentumDecision?.passed)
    .map(item => {
      const quote = quoteByCode.get(item.code);
      return {
        ...item,
        ...(quote ? {
          price:quote.price, change:quote.change, changePct:quote.changePct,
          high:quote.high, low:quote.low, open:quote.open, prevClose:quote.prevClose,
          amount:quote.amount, fetchedAt:quote.fetchedAt || item.fetchedAt
        } : {}),
        industry:resolveRecommendationIndustry(item)
      };
    }).filter(item => item.industry !== '行业待确认').map(attachRecommendationMetadata);
  ...
  result.recommendationCoverage = {
    ...
    strictQualified: result.recommendations.filter(item => !['观察候选','环境观察'].includes(item.recommendationTier)).length,
    watchQualified: result.recommendations.filter(item => ['观察候选','环境观察'].includes(item.recommendationTier)).length,
    signals,
    cachedFallback: true,
    cachedAt: cachedOverview.fetchedAt || ''
  };
  if (!cachedModelCurrent) {
    result.recommendationCoverage.momentumCandidates = null;
    result.recommendationCoverage.momentumQualified = null;
    result.recommendationCoverage.momentumUnavailableReason = '最近成功推荐属于旧模型，方案B需要重新计算';
  }
...
function buildIndividualInvestmentAnalysis({ technical, financial, newsContext, quote, marketOverview, marketNewsContext = null, fundFlowPeriod = null, subject = null }) {
  ...
  const overseas = marketOverview?.overseas;
  const overseasRiskAdjustment = overseas?.available && !overseas.stale ? Number(overseas.riskAdjustment) || 0 : 0;
  ...
  const horizonSubject = {
    ...(subject || {}),
    price:quote?.price ?? technical?.latestPrice,
    analysis:technical,
    technicalScore:technical?.score,
    qualityScore:financial?.quality?.score ?? null,
    financialAnalysis:financial,
    canslim,
    fundFlowPeriod,
    newsLabel:newsAssessment.label,
    rotationProfiles:resolveStockRotationProfiles(subject || {}, marketOverview?.sectors || []),
    entryAssessment:technical?.entryAssessment
  };
  const holdingProfile = assessRecommendationHorizons(horizonSubject);
  const dataConfidence = recommendationDataConfidence({
    ...horizonSubject,
    factorAnalysis:{available:factors.filter(item => item.available).length,total:factors.length},
    newsContext
  });
  return {
    canslim,
...
function evaluateRecommendationRisk(item, riskProfile) {
  ...
  return {
    ...
    item: {
      ...
      score: signalScore,
      signalScore,
      scoreCard: item.scoreCard ? { ...item.scoreCard, recommendation: signalScore } : item.scoreCard,
      riskProfile,
      riskUnverified: true,
...
async function fetchSinaMarketNews() {
  ...
  return sortNewsNewestFirst((json?.result?.data || []).map(row => ({
  ...
  })).filter(item => item.title && item.link && isMarketWideNews(item))).slice(0, 8);
}
...
async function fetchMarketNews(force = false) {
  ...
  const addSource = (news, source) => {
    news = news.filter(isMarketWideNews);
  ...
  try {
    const news = (await fetchEastmoneyNews(keyword, 'market-overview-news', maxAgeMs, force)).filter(isMarketWideNews);
  ...
  try {
    const news = (await fetchBingNews(keyword, 'market-overview-news-bing', maxAgeMs, force)).filter(isMarketWideNews);
...
function buildBackgroundWatchRecommendations(scored, screening, tradeDate, now = Date.now()) {
  ...
  return screening.candidates.map(candidate => {
    ...
    const item = {
      ...
      strategyDecision:{status:'watch',primaryStrategyId:candidate.primaryStrategyId,stage:candidate.stage,horizon:candidate.horizon,score:signalScore,matches:[]}
    };
    item.scoreCard = buildStrategyScoreCard(item, {snapshotId:screening.analysisId,tradeDate,observedAt:screening.generatedAt});
    item.analysisViewModel = buildAnalysisViewModel(item, {decision:item.strategyDecision,evaluations:[],view:'recommendation'});
    return item;
  }).filter(Boolean);
}

function selectMarketRecommendationCandidates({ rotationScreened = [], momentumScreened = [], breakoutScreened = [], consolidationScreened = [], reboundScreened = [], fullMarketScreened = [] }, limit = 120) {
  const selectByIndustry = (source, sourceLimit, perIndustry) => {
    const output = [];
    ...
    for (const item of source) {
      ...
      output.push(item);
      if (output.length >= sourceLimit) break;
    }
    return output;
  };
  const queues = [
    rotationScreened, momentumScreened,
    selectByIndustry(breakoutScreened, 50, 3),
    selectByIndustry(consolidationScreened, 50, 3),
    selectByIndustry(reboundScreened, 120, 4),
    selectByIndustry(fullMarketScreened, 60, 3)
  ];
  const selected=[];
  for(let index=0;index<Math.max(...queues.map(queue=>queue.length));index++) {
    queues.forEach((queue, source) => { if(queue[index]) selected.push({item:queue[index], source}); });
  }
  const remaining = [];
  const candidatesByCode = new Map();
  const priorityCodes = new Set([...rotationScreened, ...momentumScreened].map(item => item.code));
  const priorityCandidates = [];
  for (const {item, source} of selected) {
    const existing = candidatesByCode.get(item.code);
    if (existing) {
      existing.sources.add(source);
      continue;
    }
    const group = recommendationIndustryGroupKey(item);
    const candidate = {item:{ ...item, recommendationGroup: group }, sources:new Set([source])};
    candidatesByCode.set(item.code, candidate);
    if (priorityCodes.has(item.code)) {
      priorityCandidates.push(candidate.item);
      continue;
    }
    remaining.push(candidate);
  }
  // Preserve priority candidates, then balance industry exposure and screening sources.
  const candidates = priorityCandidates.slice(0, limit);
  const industryCounts = new Map();
  const sourceCounts = new Map();
  const sourceUse = candidate => Math.min(...[...candidate.sources].map(source => sourceCounts.get(source) || 0));
  candidates.forEach(item => industryCounts.set(item.recommendationGroup, (industryCounts.get(item.recommendationGroup) || 0) + 1));
  while (candidates.length < limit && remaining.length) {
    let bestIndex = 0;
    for (let index = 1; index < remaining.length; index++) {
      const current = remaining[index], best = remaining[bestIndex];
      const industryDifference = (industryCounts.get(current.item.recommendationGroup) || 0) - (industryCounts.get(best.item.recommendationGroup) || 0);
      const sourceDifference = sourceUse(current) - sourceUse(best);
      if (industryDifference < 0 || industryDifference === 0 && sourceDifference < 0) bestIndex = index;
    }
    const {item, sources} = remaining.splice(bestIndex, 1)[0];
    const source = [...sources].sort((a, b) => (sourceCounts.get(a) || 0) - (sourceCounts.get(b) || 0))[0];
    candidates.push(item);
    industryCounts.set(item.recommendationGroup, (industryCounts.get(item.recommendationGroup) || 0) + 1);
  ...
  for (const item of candidates) {
    ...
    selected.push({
      ...
      entryAssessment:{
        ...
        evidence:[...new Set([...(item.momentumDecision.entryAssessment.evidence || []),...(item.entryAssessment?.evidence || [])])]
      },
...
async function buildMarketRecommendations(marketQuotes, force = false, outcomeProfile = null, marketContext = null, onProgress = () => {}) {
  ...
  const marketAssessmentContext = {
    ...
    marketNewsContext: marketContext?.marketNewsContext || null,
    sectors: marketContext?.sectors || [],
    marketRegime:marketContext?.marketRegime || null,
    observationPhase,
    snapshotId
  };
  let directory = [];
  ...
  const scored = (marketQuotes || []).map(item => {
    ...
    const scoredItem = { ...item, industry, preliminaryScore, reboundPreliminaryScore, consolidationPreliminaryScore: scoreConsolidationCandidate(item) };
    const rotationProfiles = resolveStockRotationProfiles(scoredItem, marketAssessmentContext.sectors);
    return { ...scoredItem, rotationProfiles, momentumDecision:momentumRecommendationDecision({ ...scoredItem, rotationProfiles }) };
  }).filter(item => !/ST|退/.test(item.name) && item.price > 0 && item.high > 0);
  const factorContext = buildRecommendationFactorContext(scored);
  if (!marketAssessmentContext.sectors.length) {
    marketAssessmentContext.sectors = [...factorContext.sectorProfiles.values()].map(item => ({
      name:item.name,
      changePct:item.averageChangePct,
      upRatio:item.upRatio,
      rotationScore:item.score,
      rotationState:item.rotationState,
      capitalEstimated:true
    }));
  }
  ...
  const rotationScreened = selectRotationPriorityCandidates(scored, marketAssessmentContext.sectors, {limit:48, perSector:4, sectorLimit:12});
  ...
  const historyResults = await settleWithConcurrency(candidates, 8, async quote => {
      ...
      const analysis = {
        ...analyzeHistory(history, {observationPhase}),
  ...
  const currentFunds = new Map(currentFundResults.filter(result => result.status === 'fulfilled').flatMap(result => result.value)
    .map(row => [row.f12, { ...normalizeEastmoneyFundFlow(row),
  ...
  const recommendationResults = await settleWithConcurrency(ranked, 6, async item => {
    const [fundFlowResult, newsResult, financialResult] = await Promise.allSettled([
      fetchDailyFundFlowHistory(item.code, 10, force),
    ...
    const fundFlowPeriod = mergeFundFlowSnapshot(historicalFlow, currentFunds.get(item.code) || null, item.analysis.tradeDate);
    ...
    const breakoutSignal = !momentumSignal && !['已反弹', '底部待反弹'].includes(item.signal);
    ...
    const newsContext = newsResult.status === 'fulfilled'
      ? summarizeNews(newsResult.value.news, windowStart, { subject: item.name, code: item.code, lifecycleKey:`stock:${item.code}`, ...newsResult.value })
      : summarizeNews([], windowStart);
    const contextualAnalysis = applyEntryContextAssessment(capitalAnalysis, {
      newsContext, marketNewsContext: marketAssessmentContext.marketNewsContext,
      marketOverview: marketAssessmentContext, subject: item
    });
    const entryAnalysis = applyOutcomeFeedbackAssessment(contextualAnalysis, outcomeProfile, signal);
    const entryAssessment = entryAnalysis.entryAssessment;
    if (!momentumSignal && entryAssessment?.setupType === 'bottom-accumulation') signal = '底部吸筹';
    else if (!momentumSignal && entryAssessment?.setupType === 'sideways-washout') signal = '震荡洗盘';
    item = { ...item, signal };
    ...
    const technicalReason = breakoutSignal
      ...
      : momentumSignal ? item.momentumDecision?.entryAssessment?.summary || '板块强势扩散，等待回踩确认'
        : `${item.analysis.reboundReason}；${item.analysis.trendContinuation?.summary || ''}`;
    const newsAssessment = assessRecommendationNewsConfirmation(item, newsContext);
    ...
    const rawSignalScore = Math.max(0, Math.min(100, (breakoutSignal || momentumSignal ? item.analysis.score : item.analysis.reboundScore)
      + setupAdjustment + horizontalAdjustment));
    const qualityScore = finiteNumber(item.quality?.score);
    ...
    const { estimatedFundFlow: _estimatedFundFlow, ...publicItem } = item;
    return attachRecommendationMetadata({
      ...publicItem,
      verdict,
      signal,
      ...
      recommendationContext: {
        ...
        sector: item.rotationProfiles?.[0] ? {
          name:item.rotationProfiles[0].name, tradeDate:item.rotationProfiles[0].capitalTradeDate,
          phase:item.rotationProfiles[0].capitalTrend?.phase, score:item.rotationProfiles[0].rotationScore,
          mainNetInflow:item.rotationProfiles[0].mainNetInflow, mainNet3:item.rotationProfiles[0].mainNet3,
          mainNet5:item.rotationProfiles[0].mainNet5, mainNet10:item.rotationProfiles[0].mainNet10,
          estimated:Boolean(item.rotationProfiles[0].capitalEstimated), stale:Boolean(item.rotationProfiles[0].capitalStale)
        } : null,
        capital: fundFlowPeriod?.available ? {
          available: true, days: fundFlowPeriod.days, mainNetInflow: fundFlowPeriod.mainNetInflow,
          netRatio: fundFlowPeriod.netRatio, positiveDays: fundFlowPeriod.positiveDays, estimated:Boolean(fundFlowPeriod.estimated), source: fundFlowPeriod.source
        } : { available: false },
        news: {
        ...
        marketNews: marketAssessmentContext.marketNewsContext?.available ? {
          available:true, signal:marketAssessmentContext.marketNewsContext.signal,
          freshness:marketAssessmentContext.marketNewsContext.freshness,
          latestPublishedAt:marketAssessmentContext.marketNewsContext.latestPublishedAt,
          source:marketAssessmentContext.marketNewsContext.source
        } : { available:false }
      },
      reason,
      newsContext
  ...
  const outcomeGateFailures = recommendations.reduce((counts, item) => {
    const decision = recommendationGateDecision(item);
  ...
  const commonQualityRecommendations = recommendations.filter(item => (item.qualityScore === null
    ...
    && !['破位', '爆量观察', '结构偏弱', '不宜追高', '公司风险', '消息风险待确认', '大盘消息风险待确认', '美股科技风险待确认', '板块退潮，等待确认'].includes(item.entryAssessment?.status)
    && !(item.newsLabel === '消息谨慎' && item.signalScore < 65)
    && !(item.rotationProfiles?.[0]?.capitalEstimated === false && !item.rotationProfiles[0].capitalStale && Number(item.rotationProfiles[0].mainNetInflow) < 0 && Number(item.rotationProfiles[0].mainNetPct || 0) < 0)
    && !analyzeCapitalWindows(item.fundFlowPeriod, item.analysis?.tradeDate).weakening
    && !(item.fundFlowPeriod?.available && item.fundFlowPeriod.netRatio <= (item.fundFlowPeriod.estimated ? -6 : -3) && finiteNumber(item.fundFlowPeriod.positiveDays) !== null && item.fundFlowPeriod.positiveDays <= 4)
  ...
  const momentumQualityRecommendations = recommendations.filter(item => item.momentumDecision?.passed
    ...
    && !analyzeCapitalWindows(item.fundFlowPeriod, item.analysis?.tradeDate).weakening
    && (item.qualityScore === null ? item.dataConfidence?.score >= 50 : item.qualityScore >= 55)
    && item.newsLabel !== '消息谨慎'
    && !['破位', '爆量观察', '结构偏弱', '公司风险', '板块退潮，等待确认', '美股科技风险待确认', '大盘消息风险待确认', '消息风险待确认'].includes(item.entryAssessment?.status));
  ...
  const stableQualityCodes = new Set(stableQualityRecommendations.map(item => item.code));
  const stableRiskApprovedRecommendations = riskApprovedRecommendations.filter(item => stableQualityCodes.has(item.code));
  const strictRiskApprovedRecommendations = stableRiskApprovedRecommendations.filter(recommendationPassesOutcomeGate);
  const watchRiskApprovedRecommendations = stableRiskApprovedRecommendations.filter(item =>
    !recommendationPassesOutcomeGate(item) && recommendationPassesWatchGate(item, true));
  const stableResolution = filterResolvedRecommendations(finalizeRecommendationDisplay(
    strictRiskApprovedRecommendations, watchRiskApprovedRecommendations));
  const finalRiskApprovedRecommendations = stableResolution.items;
  const momentumResolution = filterResolvedRecommendations(finalizeMomentumRecommendations(riskApprovedRecommendations.filter(item =>
    item.momentumDecision?.passed && item.newsLabel !== '消息谨慎'
    && item.riskProfile?.status !== 'risk')));
  let momentumRecommendations = momentumResolution.items;
  const riskScoreRejected = stableRiskApprovedRecommendations.length
    - strictRiskApprovedRecommendations.length - watchRiskApprovedRecommendations.length;
  ...
  let groupedRecommendations = groupRecommendationsByIndustry(balancedRecommendations)
  ...
  const rejectedCandidates = [
    ...
    ...momentumResolution.rejected,
    ...recommendationResults.map((result, index) => result.status === 'rejected'
      ? {code:ranked[index]?.code || '', reason:'enrichment-failed', detail:result.reason?.message || String(result.reason)}
      : null).filter(Boolean),
    ...recommendations.filter(item => !selectedCodesForLedger.has(item.code)).map(item => {
      const gateDecision = recommendationGateDecision(item);
      return {
        code:item.code,
  ...
  const strategyExperimentRows = [...groupedRecommendations, ...momentumRecommendations];
  const strategyMatches = {};
  const strategyStatuses = {};
  strategyExperimentRows.forEach(item => {
    const matchedIds = (item.strategyEvaluations || []).filter(row => row.matched).map(row => row.id);
    if (!matchedIds.length && item.strategyDecision?.primaryStrategyId) matchedIds.push(item.strategyDecision.primaryStrategyId);
    [...new Set(matchedIds)].forEach(id => { strategyMatches[id] = (strategyMatches[id] || 0) + 1; });
    const status = item.strategyDecision?.status || item.recommendationTier || 'unverified';
    strategyStatuses[status] = (strategyStatuses[status] || 0) + 1;
  });
  return {
    ...
    coverage: {
      ...
      rejectedCandidates:[...new Map(rejectedCandidates.map(item => [`${item.code}:${item.reason}`, item])).values()],
      ...
      riskUnverifiedIncluded,
      riskScoreRejected,
      directSectorCapital:marketAssessmentContext.sectors.filter(item => !item.capitalEstimated && !item.capitalStale && finiteNumber(item.mainNetInflow) !== null).length,
      ...
      observationPhase,
      marketRegime:marketAssessmentContext.marketRegime,
      factorModel: '全市场横截面因子 + 独立策略注册表 + 统一风险裁决 + 板块资金轮动 + 实时消息与海外风险修正（三池独立，缺失项不计分）',
...
function marketAnalysis(result) {
  const {up,down,label:sentiment} = assessMarketEnvironment(result);
  const regimeEvidence = result.marketRegime?.evidence?.length ? `${result.marketRegime.evidence.join('；')}。` : '';
  const leaders = (result.sectors || []).slice(0, 3).map(item => `${item.name}${item.changePct >= 0 ? '+' : ''}${item.changePct.toFixed(2)}%${item.rotationState ? `（${item.rotationState}，上涨占比${(Number(item.upRatio || 0) * 100).toFixed(0)}%）` : ''}`).join('、');
  const funds = (result.fundSectors || []).slice(0, 3).map(item => `${item.name}${item.rotationState ? `（${item.rotationState}` : '（'}${item.capitalEstimated === false ? `，主力净${Number(item.mainNetInflow) >= 0 ? '流入' : '流出'}${formatCapitalAmount(Math.abs(Number(item.mainNetInflow || 0)))}，占比${signedPercent(item.mainNetPct)}` : '，量价活跃度估算'}）`).join('、');
  ...
  const momentumNames = (result.momentumRecommendations || []).slice(0, 4).map(item => item.name).join('、');
  return `市场状态${sentiment}${up || down ? `，上涨${up}家、下跌${down}家` : ''}。${regimeEvidence}${leaders ? `轮动靠前：${leaders}。` : ''}${funds ? `主力资金靠前：${funds}。` : ''}涨停${result.limits?.upCount ?? 0}只、跌停${result.limits?.downCount ?? 0}只。${overseas}${news}${signalGroups ? `稳健轮动候选——${signalGroups}；` : '当前未筛出满足条件的稳健候选。'}${momentumNames ? `强势追踪候选：${momentumNames}，仅等待回踩确认，不追涨停或爆量加速。` : '当前没有通过真实板块资金确认的强势追踪候选。'}`;
}
...
async function fetchMarketOverview(force = false, favoriteOutcomes = [], onProgress = () => {}) {
  ...
  let outcomeProfile = summarizeRecommendationOutcomes(mergeRecommendationOutcomeQuotes(favoriteOutcomes, freshCachedQuotes), {requireFreshQuote:true});
  ...
  if (!force && marketOverviewCache && Date.now() - marketOverviewCache.savedAt < 30 * 1000
    && canReuseMarketRecommendations(marketOverviewCache.value, { feedbackSignature })) {
  ...
  if (snapshotResult.status === 'fulfilled') {
    ...
    outcomeProfile = summarizeRecommendationOutcomes(mergeRecommendationOutcomeBenchmarks(
      mergeRecommendationOutcomeQuotes(favoriteOutcomes, outcomeQuotes), result.indices || []), {requireFreshQuote:true});
    feedbackSignature = outcomeProfile.signature;
    let reuseRecommendations = false;
    ...
    result.tradeDate = snapshotResult.value.quotes.map(row=>row.tradeDate).filter(Boolean).sort().at(-1) || '';
    result.turnover = snapshotResult.value.turnover;
    result.errors.push(...snapshotResult.value.errors.map(error => `全市场行情部分失败：${error}`));
    const newsPromise = fetchMarketNews(force);
    const marketNewsContextPromise = newsPromise.then(value => summarizeNews(value.news, '', { ...value, scope:'market', lifecycleKey:'market' })).catch(() => summarizeNews([]));
    const [capitalResult, sectorsResult] = await Promise.allSettled([
      fetchSectorCapitalFlow(force),
      fetchTencentMarketSectors(snapshotResult.value.quotes)
    ]);
    const capitalRows = capitalResult.status === 'fulfilled' ? capitalResult.value.rows : [];
    if (capitalResult.status === 'fulfilled') {
      result.sectorCapital = {
        direct:capitalResult.value.direct,
        source:capitalResult.value.source,
        fetchedAt:capitalResult.value.fetchedAt,
        cached:Boolean(capitalResult.value.cached),
        stale:Boolean(capitalResult.value.stale)
      };
      result.warnings.push(...(capitalResult.value.warnings || []).map(error => `板块资金来源切换：${error}`));
    } else {
      result.warnings.push(`板块真实资金失败，已使用涨幅、广度和成交活跃度估算：${capitalResult.reason?.message || capitalResult.reason}`);
    }
    if (sectorsResult.status === 'fulfilled') {
      const mergedSectors = capitalRows.length
        ? mergeSectorCapitalRows(sectorsResult.value, capitalRows, snapshotResult.value.quotes)
        : sectorsResult.value;
      Object.assign(result, sectorsResult.value, mergedSectors);
    }
    ...
    feedbackSignature = outcomeProfile.signature;
    const hasDirectSectorCapital = result.sectors.some(item => !item.capitalEstimated && !item.capitalStale && finiteNumber(item.mainNetInflow) !== null);
    const marketRegimeSignature = [result.marketRegime?.key, ...(result.marketRegime?.strongSectors || [])].join('|');
    reuseRecommendations = !force && canReuseMarketRecommendations(recommendationCache, { feedbackSignature, hasDirectSectorCapital, marketRegimeSignature });
    const [recommendationsResult, newsResult] = await Promise.allSettled([
      reuseRecommendations ? Promise.resolve(null) : (async () => buildMarketRecommendations(snapshotResult.value.quotes, force, outcomeProfile, {
        indices: result.indices || [], overseas: result.overseas,
        marketNewsContext: await marketNewsContextPromise,
        sectors:result.sectors || [], marketRegime:result.marketRegime,
        observationPhase:result.observationPhase, snapshotId:result.snapshotId
      }, onProgress))(),
      newsPromise
    ]);
    if (recommendationsResult.status === 'fulfilled' && recommendationsResult.value) {
      ...
      result.recommendationCoverage.marketRegimeSignature = marketRegimeSignature;
      if (result.recommendationCoverage.fundFlowErrors?.length) {
        result.warnings.push(`部分候选每日资金明细未取得；真实多日汇总 ${result.recommendationCoverage.fundFlowAggregate || 0} 只，量价代理 ${result.recommendationCoverage.fundFlowEstimated || 0} 只，不推算每日流入天数`);
      }
    ...
    if (newsResult.status === 'fulfilled') {
      result.newsContext = summarizeNews(newsResult.value.news, '', { ...newsResult.value, scope:'market', lifecycleKey:'market' });
  ...
  result.momentumRecommendations ||= [];
  ...
  if (!result.indices.length && !result.sectors.length) {
    if (previousOverview?.indices?.length || previousOverview?.sectors?.length) {
      ...
      fallback.recommendations = (fallback.recommendations || [])
        .map(item => ({ ...deactivateCachedRecommendation(item), industry: resolveRecommendationIndustry(item) }))
        .filter(item => item.industry !== '行业待确认')
        .map(attachRecommendationMetadata);
      fallback.momentumRecommendations = [];
      if (fallback.recommendationCoverage?.modelVersion !== RECOMMENDATION_MODEL_VERSION) {
        fallback.momentumRecommendations = [];
        fallback.recommendationCoverage = {
          ...(fallback.recommendationCoverage || {}),
          momentumCandidates:null,
          momentumQualified:null,
          momentumUnavailableReason:'最近成功推荐属于旧模型，方案B需要重新计算'
        };
      }
...
async function fetchBoardStocks(board) {
  ...
  try {
    ...
    const buildUrl = page => `https://82.push2.eastmoney.com/api/qt/clist/get?pn=${page}&pz=100&po=1&np=1&fltt=2&invt=2&fid=f3&fs=${encodeURIComponent(`b:${board.code}`)}&fields=${fields}&_=${Date.now()}`;
    let result;
    try {
      result = await fetchEastmoneyPages(buildUrl, 6);
    } catch (primaryError) {
      result = await fetchEastmoneyPages(page => buildUrl(page).replace('82.push2.eastmoney.com', 'push2.eastmoney.com'), 6);
      result.errors.unshift(`82.push2.eastmoney.com: ${primaryError.message || primaryError}`);
    }
...
function emF10Code(code) {
  const value = String(code || '');
  const market = marketPrefixOf(value).toUpperCase();
  return `${market}${value}`;
}
...
function estimateFundFlowFromHistory(history, days = 10) {
  const recent = (history || []).filter(row => Number(row?.close) > 0 && Number(row?.high) > 0 && Number(row?.low) > 0)
    .slice(-Math.max(1, days)).map(row => {
      const high = Number(row.high);
      const low = Number(row.low);
      const close = Number(row.close);
      const open = Number(row.open) || close;
      const range = high - low;
      const pressure = range > 0 ? ((close - low) - (high - close)) / range : close >= open ? .2 : -.2;
      const amount = Number(row.amount) > 0 ? Number(row.amount) : close * Math.max(0, Number(row.volume) || 0) * 100;
      return { date:String(row.date || ''), amount, pressure, mainNetInflow:amount * Math.max(-1, Math.min(1, pressure)) };
    }).filter(row => row.date && row.amount > 0);
  if (!recent.length) return { available:false, estimated:true, days:0, rows:[], source:'日线量价资金强度估算（非Level-2主力口径）' };
  const mainNetInflow = recent.reduce((sum, row) => sum + row.mainNetInflow, 0);
  const totalAmount = recent.reduce((sum, row) => sum + row.amount, 0);
  return {
    available:true,
    estimated:true,
    days:recent.length,
    ...
    mainOutflow:null,
    mainNetInflow,
    netRatio:totalAmount ? mainNetInflow / totalAmount * 100 : 0,
    grossFlowsAvailable:false,
    positiveDays:recent.filter(row => row.mainNetInflow > 0).length,
    rows:recent,
    source:'日线量价资金强度估算（非Level-2主力口径）'
  };
}
...
async function fetchDataCenterFinancialRows(code) {
  ...
  const json = await getJsonWithRetry(`https://datacenter-web.eastmoney.com/api/data/v1/get?${params}`, 1);
  const rows = dataCenterRows(json);
...
function mapDataCenterCompanyProfile(data = {}) {
  return {industry:cleanStockName(data.EM2016 || data.BOARD_NAME_LEVEL || data.INDUSTRYCSRC1 || ''),
    business:plainText(data.MAIN_BUSINESS || data.BUSINESS_SCOPE || ''), summary:plainText(data.ORG_PROFILE || '')};
}

function formatBusinessProducts(rows) {
  const dated = (rows || []).filter(row=>row?.ITEM_NAME || row?.PRODUCTNAME || row?.name);
  const latest = dated.map(row=>String(row.REPORT_DATE || '')).sort().at(-1);
  const current = dated.filter(row=>String(row.REPORT_DATE || '') === latest);
  const products = current.filter(row=>String(row.MAINOP_TYPE) === '2');
  const industries = current.filter(row=>String(row.MAINOP_TYPE) === '1');
  const selected = products.length ? products : industries.length ? industries : current;
  const details = selected.sort((a,b)=>Number(b.MAIN_BUSINESS_INCOME || b.INCOME || b.YYSR || 0)-Number(a.MAIN_BUSINESS_INCOME || a.INCOME || a.YYSR || 0))
    .slice(0,8).map(row=>{
      const income = finiteNumber(row.MAIN_BUSINESS_INCOME ?? row.INCOME ?? row.YYSR);
      const ratio = finiteNumber(row.MBI_RATIO);
      return [plainText(row.ITEM_NAME || row.PRODUCTNAME || row.name), income !== null ? `收入${formatCapitalAmount(income)}` : '',
        ratio !== null ? `占比${(ratio*100).toFixed(2)}%` : ''].filter(Boolean).join('，');
    }).join('；');
  return details ? `${selected[0]?.REPORT_NAME || latest?.slice(0,10) || ''} ${details}`.trim() : '';
}

function dataCenterRows(json) {
  if (json?.code === 9201 && json.message === '返回数据为空') return [];
  if (!json?.success || !Array.isArray(json.result?.data)) throw new Error(json?.message || '数据中心响应无效');
  return json.result.data;
}

async function fetchDataCenterCompanyRows(reportName, code) {
  const params = new URLSearchParams({reportName,columns:'ALL',filter:`(SECUCODE="${emF10Code(code).slice(2)}.${emF10Code(code).slice(0,2)}")`,
    pageSize:'100',pageNumber:'1',...(reportName === 'RPT_F10_FN_MAINOP' ? {sortColumns:'REPORT_DATE',sortTypes:'-1'} : {})});
  const json = await getJsonWithRetry(`https://datacenter-web.eastmoney.com/api/data/v1/get?${params}`,1);
  const rows = dataCenterRows(json);
  if (!rows.length) throw new Error('公司资料为空');
  return rows;
}
...
async function fetchCompanyProfile({ code, name, sector, force = false }) {
  ...
  if (cached) return { ...cached, cached: true };
  const previous = readDiskCache(`company-profile-${cacheKey}`,180 * 24 * 60 * 60 * 1000);
  const errors = [];
  const profile = {
    code,
  ...
  const dataCenter = await Promise.allSettled([
    fetchDataCenterCompanyRows('RPT_F10_BASIC_ORGINFO',code), fetchDataCenterCompanyRows('RPT_F10_FN_MAINOP',code)
  ]);
  if (dataCenter[0].status === 'fulfilled') {
    Object.assign(profile,mapDataCenterCompanyProfile(dataCenter[0].value[0]));
    profile.source = '东方财富数据中心公司资料';
  } else errors.push(`数据中心公司资料：${dataCenter[0].reason?.message || dataCenter[0].reason}`);
  if (dataCenter[1].status === 'fulfilled') {
    profile.products = formatBusinessProducts(dataCenter[1].value);
    profile.source = [profile.source,'东方财富数据中心主营构成'].filter(Boolean).join(' + ');
  } else errors.push(`数据中心主营构成：${dataCenter[1].reason?.message || dataCenter[1].reason}`);
  const requests = await settleWithConcurrency([
    () => fetchStockBoards(code),
    () => profile.business && profile.summary && profile.industry ? null : getJsonWithRetry(`https://emweb.securities.eastmoney.com/PC_HSF10/CompanySurvey/PageAjax?code=${encodeURIComponent(emF10Code(code))}`, 1),
    () => profile.business && profile.summary ? null : getJsonWithRetry(`https://emweb.securities.eastmoney.com/PC_HSF10/OperationsRequired/PageAjax?code=${encodeURIComponent(emF10Code(code))}`, 1),
    () => profile.products ? null : getJsonWithRetry(`https://emweb.securities.eastmoney.com/PC_HSF10/BusinessAnalysis/PageAjax?code=${encodeURIComponent(emF10Code(code))}`, 1)
  ], 2, request => request());
  try {
    if (requests[0].status === 'rejected') throw requests[0].reason;
    profile.tags = requests[0].value;
    profile.industry ||= profile.tags[0] || '';
    if (profile.tags.length) profile.source = [profile.source,'东方财富板块归属'].filter(Boolean).join(' + ');
  } catch (err) {
  ...
  try {
    ...
    profile.industry ||= cleanStockName(data.SSHY || data.INDUSTRY || data.HY || data.BK || '');
    ...
    if (survey) profile.source = [profile.source, '东方财富F10'].filter(Boolean).join(' + ');
  } catch (err) {
  ...
  try {
    ...
    if (required && (profile.business || profile.summary)) profile.source = [profile.source, '东方财富F10操盘必读'].filter(Boolean).join(' + ');
  } catch (err) {
  ...
  try {
    ...
    profile.products ||= formatBusinessProducts(productRows);
    if (profile.products && !profile.source) profile.source = '东方财富F10';
  } catch (err) {
    errors.push(`经营分析失败：${err.message || err}`);
  }
  const cachedFields = [];
  for (const field of ['industry','business','summary','products']) {
    if (!profile[field] && previous?.profile?.[field]) {
      profile[field] = previous.profile[field];
      cachedFields.push(field);
    }
  }
  if (cachedFields.length) profile.source += ` + 公司资料缓存（${previous.fetchedAt}）`;
  const missingFields = ['industry','business','summary','products'].filter(field=>!profile[field]);
  appendLogLine({type:missingFields.length || cachedFields.length ? 'warn' : 'success',action:'company_data_coverage',message:'公司资料完整性核验',detail:{code,source:profile.source,missingFields,cachedFields,sourceErrors:errors}});
  const result = { profile, errors:missingFields.length || cachedFields.length ? errors : [],
    warnings:missingFields.length || cachedFields.length ? [] : errors, missingFields, cachedFields, stale:cachedFields.length > 0,
    fetchedAt:cachedFields.length ? previous.fetchedAt : new Date().toISOString(), checkedAt:new Date().toISOString() };
  if (!missingFields.length && !cachedFields.length) writeDiskCache(`company-profile-${cacheKey}`,result);
  return writeTimedCache(companyProfileCache, cacheKey, result);
}
...
function summarizeEastmoneyFundHistory(klines, days = 10) {
  const byDate = new Map();
  for (const line of klines || []) {
    const cols = String(line).split(',');
    if (!/^\d{4}-\d{2}-\d{2}$/.test(cols[0]) || finiteNumber(cols[1]) === null) continue;
    byDate.set(cols[0], {date:cols[0], mainNetInflow:finiteNumber(cols[1]), mainNetPct:finiteNumber(cols[6]), mainInflow:null, mainOutflow:null});
  }
  const rows = [...byDate.values()].sort((a,b)=>a.date.localeCompare(b.date)).slice(-days);
  const ratios = rows.map(row=>row.mainNetPct).filter(value=>value !== null);
  return {available:rows.length > 0, estimated:false, days:rows.length, requestedDays:days, complete:rows.length >= days,
    startDate:rows[0]?.date || '', endDate:rows.at(-1)?.date || '', rows,
    mainNetInflow:rows.length ? rows.reduce((sum,row)=>sum+row.mainNetInflow,0) : null,
    netRatio:ratios.length ? average(ratios) : null, mainInflow:null, mainOutflow:null, grossFlowsAvailable:false,
    positiveDays:rows.filter(row=>row.mainNetInflow > 0).length};
}

async function fetchDailyFundFlowHistory(code, days = 10, force = false) {
  const cacheKey = `daily:${code}:${days}`;
  const cached = force ? null : readTimedCache(stockFundFlowHistoryCache, cacheKey, 2 * 60 * 1000);
  if (cached) return {...cached, cached:true};
  const params = new URLSearchParams({secid:secidOf(code), lmt:String(days), klt:'101', fields1:'f1,f2,f3,f7',
    fields2:'f51,f52,f53,f54,f55,f56,f57,f58,f59,f60,f61,f62,f63,f64,f65', ut:'b2884a393a59ad64002292a3e90d46a5'});
  const sources = [
    {name:'东方财富历史资金', load:async()=>{
      const json = JSON.parse(await requestText(`https://push2his.eastmoney.com/api/qt/stock/fflow/daykline/get?${params}`, 3500));
      if (json.rc !== 0 || json.data?.code !== code) throw new Error('历史资金响应无效');
      return summarizeEastmoneyFundHistory(json.data.klines, days);
    }},
    {name:'新浪历史资金流向', load:()=>fetchSinaFundFlowHistory(code,days,force)},
    {name:'东方财富访客逐日资金', load:async()=>{
      const json = JSON.parse(await requestText(`https://push2.eastmoney.com/webguest/api/qt/stock/fflow/daykline/get?${params}`, 3500));
      if (json.rc !== 0 || json.data?.code !== code) throw new Error('访客资金响应无效');
      return summarizeEastmoneyFundHistory(json.data.klines,days);
    }}
  ];
  const sourceErrors = [];
  ...
  for (const source of sources) {
    try {
      ...
      if (!value.available) throw new Error('未返回有效逐日明细');
      if (!best || value.endDate > best.endDate || value.endDate === best.endDate && value.days > best.days) best = {...value, source:source.name};
      if (value.days >= days) break;
      sourceErrors.push(`${source.name}仅返回${value.days}/${days}日`);
    } catch (error) { sourceErrors.push(`${source.name}：${error.message || error}`); }
  }
  appendLogLine({type:best?.days >= days ? 'info' : 'warn', action:'daily_fund_flow', message:'逐日资金来源核验',
    detail:{code, requestedDays:days, actualDays:best?.days || 0, endDate:best?.endDate || '', source:best?.source || '', sourceErrors}});
  if (!best) throw new Error(sourceErrors.join('；'));
  return writeTimedCache(stockFundFlowHistoryCache,cacheKey,{...best, requestedDays:days, complete:best.days >= days,
    sourceErrors, fallbackReason:best.days < days ? `逐日资金仅取得${best.days}/${days}日；${sourceErrors.join('；')}` : '', fetchedAt:new Date().toISOString()});
}
...
function assessCurrentEntry({
  ...
  if (![latestPrice, supportPrice, resistance, ma5, ma10, ma20, ma30].every(value => finiteNumber(value) > 0)
    || finiteNumber(volumeRatio) === null || ratio < 0) {
  ...
  if (priceConfirmed && trendConfirmed && momentumHealthy && isControlledVolumeExpansion(ratio) && observationPhase?.phase === 'intraday') {
    return withStructure({ allowed: false, status: '盘中突破，等待收盘确认', tone: 'warning',
  ...
  if (consolidationBreakout?.available && consolidationBreakout.isConsolidating) {
    ...
    const lowBuyCandidate = looksLikeWashout && trendConfirmed && momentumHealthy && Number.isFinite(boxPosition)
...
function analyzePathMetrics(history) {
  const rows=(history || []).filter(row=>Number.isFinite(row?.close) && row.close>0);
  if(rows.length<2) return {available:false,summary:'长周期涨跌数据不足'};
  let peak=rows[0].close, trough=peak, drawdown=0, runup=0, up=0, down=0, longestUp=0, longestDown=0;
  let bestDay=-Infinity, worstDay=Infinity;
  for(let i=1;i<rows.length;i++) {
    const price=rows[i].close, change=(price/rows[i-1].close-1)*100;
    peak=Math.max(peak,price); trough=Math.min(trough,price);
    drawdown=Math.min(drawdown,(price/peak-1)*100);
    runup=Math.max(runup,(price/trough-1)*100);
    up=change>0?up+1:0; down=change<0?down+1:0;
    longestUp=Math.max(longestUp,up);longestDown=Math.max(longestDown,down);
    bestDay=Math.max(bestDay,change);worstDay=Math.min(worstDay,change);
  }
  const returns=Object.fromEntries([5,20,60,120,250].map(days=>[days,rows.length>days?roundMetric((rows.at(-1).close/rows.at(-1-days).close-1)*100):null]));
  return {available:true,observations:rows.length,startDate:rows[0].date,endDate:rows.at(-1).date,
    returns,maxDrawdown:roundMetric(drawdown),maxRunup:roundMetric(runup),rangePct:roundMetric((peak/trough-1)*100),
    bestDay:roundMetric(bestDay),worstDay:roundMetric(worstDay),longestUp,longestDown,
    summary:`可用${rows.length}根日线（${rows[0].date || '起点未知'}至${rows.at(-1).date || '终点未知'}），窗口内最大上涨${runup.toFixed(1)}%、最大收盘回撤${drawdown.toFixed(1)}%，最长连涨${longestUp}日、连跌${longestDown}日；不代表上市以来全部历史`};
}

function analyzeTrendContinuation(history) {
  const rows = history.slice(-60);
  if (rows.length < 40 || rows.some(row => !(row.close > 0) || !Number.isFinite(row.close))) {
    return { available:false, passed:false, summary:'趋势延续数据不足' };
  }
  const recent = rows.slice(-10);
  const ma20 = average(rows.slice(-20).map(row=>row.close));
  const priorMa20 = average(rows.slice(-25,-5).map(row=>row.close));
  const aboveDays = recent.filter((row,index)=> {
    const end = rows.length - 10 + index + 1;
    return row.close >= average(rows.slice(end-20,end).map(value=>value.close));
  }).length;
  let peak = recent[0].close, drawdown = 0;
  recent.forEach(row=>{ peak=Math.max(peak,row.close); drawdown=Math.min(drawdown,(row.close/peak-1)*100); });
  const distance = (rows.at(-1).close / ma20 - 1) * 100;
  const slope = (ma20 / priorMa20 - 1) * 100;
  const passed = aboveDays >= 8 && slope > 0 && drawdown >= -10 && distance >= 0 && distance <= 12
    && percentageReturn(rows,5) <= 12;
  return {
    available:true, passed, aboveDays, window:10, drawdown:roundMetric(drawdown),
    ma20Slope:roundMetric(slope), distanceToMA20:roundMetric(distance),
    summary:`近10日有${aboveDays}日站在当日MA20上方，MA20较5日前${slope >= 0 ? '抬升' : '下降'}${Math.abs(slope).toFixed(1)}%，窗口内收盘最大回撤${Math.abs(drawdown).toFixed(1)}%，现价偏离MA20 ${distance.toFixed(1)}%；${passed ? '趋势延续条件成立，仍需资金、消息和入场位置确认' : '趋势持续性或价格位置尚未满足条件'}`
  };
}

function analyzeHistory(history, options = {}) {
  if (Array.isArray(history) && history.length < 20) {
    const error = new Error(`历史样本不足：当前${history.length}根日线，策略至少需要20根；暂不生成技术推荐`);
    error.code = 'INSUFFICIENT_HISTORY';
    throw error;
  }
  if (!Array.isArray(history) || history.some(row =>
    !row || ![row.close,row.open,row.high,row.low,row.volume].every(Number.isFinite)
      || row.close <= 0 || row.low <= 0 || row.high < row.low || row.volume < 0)) {
    throw new Error('历史行情不足或包含无效价格、成交量');
  }
  const rows = history.slice(-120);
  const closes = rows.map(row => row.close);
  const latest = rows.at(-1);
  const observationPhase = options.observationPhase || {phase:'closed', label:'历史收盘', tradeDate:latest.date};
  ...
  const reboundReason = reboundSignal === '已反弹'
    ? `近60日从前高回撤${Math.abs(bottomDrawdown).toFixed(1)}%，${bottomRow.date}见阶段低点后反弹${reboundFromBottom.toFixed(1)}%；现价站上MA20，MA5高于MA10，MACD${macdHistogram > 0 ? '柱值为正' : '持续改善'}`
    : reboundSignal === '底部待反弹'
  ...
  const volumePrefix = observationPhase.phase === 'intraday' ? '盘中累计量' : '当日完整成交量';
  const volumeCaveat = observationPhase.phase === 'intraday' ? '，仅作盘中观察，不替代收盘量能确认' : '';
  const volume = isExplosiveVolume(volumeRatio) ? `${volumePrefix}为20日均量的${volumeRatio.toFixed(2)}倍，超过4.0倍，属于爆量，不作为健康突破确认${volumeCaveat}；近5日均量比为${volume5Ratio.toFixed(2)}。` : isControlledVolumeExpansion(volumeRatio) ? `${volumePrefix}为20日均量的${volumeRatio.toFixed(2)}倍，处于1.5-4.0倍放量区间${volumeCaveat}；近5日均量比为${volume5Ratio.toFixed(2)}。` : volumeRatio <= .75 ? `${volumePrefix}为20日均量的${volumeRatio.toFixed(2)}倍，当前缩量${volumeCaveat}；近5日均量比为${volume5Ratio.toFixed(2)}。` : `${volumePrefix}为20日均量的${volumeRatio.toFixed(2)}倍，未达到1.5倍放量标准${volumeCaveat}；近5日均量比为${volume5Ratio.toFixed(2)}。`;
  const entry = rising
  ...
  return {
    ...
    ma5: roundMetric(ma5), ma10: roundMetric(ma10), ma20: roundMetric(ma20), ma30: roundMetric(ma30), ma60: roundMetric(ma60), maAlignment,
...
async function fetchStockHistory({ code, name, industry = '', sector = '', force = false, favoriteOutcomes = [] }) {
  ...
  const [historyResult, quoteResult, newsResult, riskResult, financialResult, fundFlowResult, marketNewsResult, marketIndicesResult, overseasResult, sectorCapitalResult, currentFundResult] = await Promise.allSettled([
    (async () => {
      ...
      try {
        return { history: await fetchSinaHistory(cacheKey, 320), source: '新浪日线' };
      } catch (err) {
    ...
    fetchDailyFundFlowHistory(cacheKey, 10, force),
  ...
  outcomeProfile = summarizeRecommendationOutcomes(mergeRecommendationOutcomeBenchmarks(
    mergeRecommendationOutcomeQuotes(favoriteOutcomes, [...quoteCache.values()]),
    marketIndicesResult.status === 'fulfilled' ? marketIndicesResult.value : [],
    sectorCapitalResult.status === 'fulfilled' ? sectorCapitalResult.value.rows : []), {requireFreshQuote:true});
  feedbackSignature = outcomeProfile.signature;
  ...
  const fundFlowPeriod = mergeFundFlowSnapshot(historicalFlow, currentFundResult.status === 'fulfilled' ? currentFundResult.value : null, analysis.tradeDate);
  if (currentFundResult.status === 'rejected') errors.push(`当日资金失败：${currentFundResult.reason?.message || currentFundResult.reason}`);
  if (fundFlowResult.status === 'rejected') errors.push(`每日资金明细失败，${fundFlowPeriod.aggregateOnly ? '已取得真实3/5/10日汇总，未提供每日流入天数' : '已使用日线量价资金代理'}：${fundFlowResult.reason?.message || fundFlowResult.reason}`);
  else if (!fundFlowResult.value.complete) errors.push(fundFlowResult.value.fallbackReason);
  ...
  try {
    ...
    newsContext = summarizeNews(newsResult.value.news, '', { subject: name, code: cacheKey, lifecycleKey:`stock:${cacheKey}`, ...newsResult.value });
  ...
  if (marketNewsResult.status === 'fulfilled') {
    marketNewsContext = summarizeNews(marketNewsResult.value.news, '', { ...marketNewsResult.value, scope:'market', lifecycleKey:'market' });
  ...
  if (sectorCapitalResult.status === 'fulfilled') {
    analysisMarketOverview.sectors = mergeSectorCapitalRows({sectors:analysisMarketOverview.sectors || []},sectorCapitalResult.value.rows,[],analysis.tradeDate).sectors;
    errors.push(...(sectorCapitalResult.value.warnings || []).map(message=>`板块资金来源提示：${message}`));
  } else {
    analysisMarketOverview.sectors = (analysisMarketOverview.sectors || []).map(row=>({...row,capitalStale:true}));
    errors.push(`最新板块资金未确认：${sectorCapitalResult.reason?.message || sectorCapitalResult.reason}`);
  }
  const directory = stockDirectoryCache || readDiskCache('stock-directory') || [];
  const resolvedIndustry = resolveRecommendationIndustry({code:cacheKey,industry,sector},new Map(directory.map(row=>[row.code,row])));
  ...
  const detailSectorProfile = resolveStockRotationProfiles({code:cacheKey,name,industry:resolvedIndustry,sector}, analysisMarketOverview.sectors)[0] || null;
  const detailFactorAnalysis = buildDetailFactorAnalysis(analysis, fundFlowPeriod, detailSectorProfile);
  ...
  analysis.scoreCard = buildStrategyScoreCard({
    ...
    factorAnalysis:detailFactorAnalysis, dataConfidence:investmentAnalysis.dataConfidence
  }, {snapshotId, tradeDate:analysis.tradeDate, observedAt:new Date(stockObservedAt).toISOString(), observationPhase});
  investmentAnalysis.scoreCard = analysis.scoreCard;
  investmentAnalysis.factorAnalysis = detailFactorAnalysis;
  const platformDetail = attachStrategyPlatform({
    code:cacheKey,
    name,
    price:liveQuote?.price ?? analysis.latestPrice,
    analysis,
    score:analysis.score,
    signalScore:analysis.scoreCard.recommendation,
    scoreCard:analysis.scoreCard,
    factorAnalysis:detailFactorAnalysis,
    qualityScore:financialAnalysis?.quality?.score,
    riskProfile,
    holdingPeriod:analysis.holdingPeriod,
    signal:analysis.entryAssessment?.status,
    reason:analysis.entryAssessment?.summary
  ...
  investmentAnalysis.strategyDecision = analysis.strategyDecision;
  ...
  return writeTimedCache(stockHistoryCache, cacheKey, { history, analysis: { ...analysis, source, latestTradeDate, analyzedAt }, fundFlowPeriod, financialAnalysis, investmentAnalysis, riskProfile, newsContext, marketNewsContext, feedbackSignature, errors, source, latestTradeDate, analyzedAt, snapshotId, observationPhase });
}
...
async function runFullMarketResearchJob(input = {}, onProgress = () => {}) {
  ...
  if (!manifest || input.force) {
    manifest = await store.start({
      jobId,
      analysisId:input.analysisId || marketSnapshotId({tradeDate, observedAt:Date.now(), universe:snapshot.quotes.length,
        source:'full-market-research', contentSignature:marketQuoteContentSignature(snapshot.quotes)}),
      tradeDate,
      total:snapshot.quotes.length,
      modelVersion:RECOMMENDATION_MODEL_VERSION
    });
  } else if (manifest.state !== 'completed') {
    manifest = await store.resume(jobId);
  }
  ...
  for (let offset = 0; offset < remaining.length; offset += 20) {
    ...
    const rows = settled.map((result, index) => result.status === 'fulfilled' ? result.value
      : {code:batch[index].code, name:batch[index].name, tradeDate, error:result.reason?.message || String(result.reason)});
    manifest = await store.recordBatch(jobId, rows);
    onProgress({...manifest, stage:'history', message:`全市场历史任务：已完成 ${manifest.completed}/${manifest.total}`});
  }
  const records = await store.readResults(jobId);
  const successful = records.filter(item => item.factor);
  const factors = buildFactorSnapshot(successful, {
    analysisId:manifest.analysisId,
    tradeDate,
    universeSize:snapshot.quotes.length,
  ...
  const screening = {
    ...
    partial:!factors.complete,
    tradeDate,
    generatedAt:new Date().toISOString(),
    modelVersion:RECOMMENDATION_MODEL_VERSION,
    analysisId:manifest.analysisId,
    universe:snapshot.quotes.length,
    historyCovered:successful.length,
    failures:records.length - successful.length,
    codes:candidates.map(item => item.code),
    candidates
  };
  await atomicWriteJson(cacheFilePath('full-market-screening'), {savedAt:Date.now(), data:screening});
  manifest = await store.complete(jobId);
  onProgress({...manifest, stage:'complete', message:`全市场任务完成：历史覆盖 ${successful.length}/${snapshot.quotes.length}`});
  return {...manifest, historyCovered:successful.length, failures:screening.failures};
}
...
if (!isMainThread && workerData?.fullMarketResearch) {
  runFullMarketResearchJob(workerData.request || {}, progress => parentPort.postMessage({progress}))
...
function startFullMarketResearch(request = {}, sender = null) {
  if (fullMarketWorker) return fullMarketStatus || {state:'running', jobId:fullMarketJobId};
  fullMarketStatus = {state:'starting', completed:0, total:0, message:'正在启动全市场历史任务'};
  fullMarketWorker = new Worker(__filename, {workerData:{
    isPackaged:app.isPackaged,
    dataRoot:dataRootPath(),
    fullMarketResearch:true,
    request
  }});
  fullMarketWorker.on('message', message => {
    if (message.progress) {
      fullMarketStatus = message.progress;
      fullMarketJobId = message.progress.jobId || fullMarketJobId;
      if (sender && !sender.isDestroyed()) sender.send('full-market-research-progress', fullMarketStatus);
    } else if (message.result) {
      fullMarketStatus = message.result;
      fullMarketJobId = message.result.jobId || fullMarketJobId;
      if (sender && !sender.isDestroyed()) sender.send('full-market-research-progress', fullMarketStatus);
      fullMarketWorker?.terminate();
      fullMarketWorker = null;
    } else if (message.error) {
      fullMarketStatus = {...fullMarketStatus, state:'failed', error:message.error, message:'全市场历史任务失败'};
      appendLogLine({type:'error', action:'full_market_research_failed', message:'全市场历史任务失败', stack:message.error});
      if (sender && !sender.isDestroyed()) sender.send('full-market-research-progress', fullMarketStatus);
      fullMarketWorker?.terminate();
      fullMarketWorker = null;
    }
  });
  fullMarketWorker.on('error', error => {
    fullMarketStatus = {...fullMarketStatus, state:'failed', error:error.message, message:'全市场历史任务线程异常'};
    appendLogLine({type:'error', action:'full_market_worker_failed', message:'全市场历史任务线程异常', stack:error.stack || String(error)});
    fullMarketWorker = null;
  });
  fullMarketWorker.on('exit', code => {
    if (!fullMarketWorker) return;
    fullMarketWorker = null;
    if (code !== 0 && !['completed','cancelled','failed'].includes(fullMarketStatus?.state)) {
      fullMarketStatus = {...fullMarketStatus, state:'failed', message:`全市场历史任务线程退出：${code}`};
      appendLogLine({type:'error', action:'full_market_worker_exit', message:'全市场历史任务线程异常退出', detail:{code}});
    }
  });
...
ipcMain.handle('start-full-market-research', async (_event, request) => {
  appendLogLine({type:'info', action:'full_market_research_start', message:'启动或恢复全市场历史任务', detail:request || {}});
  return startFullMarketResearch(request || {}, _event?.sender);
});
...
async function loadResearchAccount() {
  return readJsonWithBackup(researchAccountFile(), null);
}

ipcMain.handle('load-research-account', async () => loadResearchAccount());

ipcMain.handle('create-research-account', async (_event, request = {}) => {
  try {
    const account = createResearchAccount({
      accountId:`research-${Date.now()}`,
      initialCash:Number(request.initialCash),
      rules:RESEARCH_ACCOUNT_RULES
    });
    await atomicWriteJson(researchAccountFile(), account);
    appendLogLine({type:'success', action:'research_account_created', message:'研究模拟账户已创建', detail:{accountId:account.accountId, initialCash:account.initialCash, rules:account.rules}});
    return {ok:true, account};
  } catch (error) {
    appendLogLine({type:'error', action:'research_account_create_failed', message:'研究模拟账户创建失败', stack:error.stack || String(error)});
    return {ok:false, error:error.message || String(error)};
  }
});

ipcMain.handle('execute-research-order', async (_event, order = {}) => {
  ...
  try {
    ...
    return {...result, recoveredFromBackup:loaded.recoveredFromBackup};
  } catch (error) {
    appendLogLine({type:'error', action:'research_order_failed', message:'研究模拟委托处理失败', detail:{order}, stack:error.stack || String(error)});
    return {ok:false, error:error.message || String(error)};
  }
});

ipcMain.handle('mark-research-account', async (_event, request = {}) => {
  const loaded = await loadResearchAccount();
  if (!loaded.value) return {ok:false, account:null};
  try {
    const account = markResearchAccount(loaded.value, Array.isArray(request.quotes) ? request.quotes : [], request.tradeDate || '');
    await atomicWriteJson(researchAccountFile(), account);
    return {ok:true, account, recoveredFromBackup:loaded.recoveredFromBackup};
  } catch (error) {
    appendLogLine({type:'error', action:'research_account_mark_failed', message:'研究模拟账户估值失败', stack:error.stack || String(error)});
    return {ok:false, error:error.message || String(error)};
  }
});

ipcMain.handle('load-user-state', async () => {
  const file = path.join(dataRootPath(), 'data', 'user-state.json');
  return readJsonWithBackup(file, null);
});

ipcMain.handle('save-user-state', async (_event, state) => {
  const file = path.join(dataRootPath(), 'data', 'user-state.json');
  try {
    const value = {schemaVersion:2, updatedAt:new Date().toISOString(), labels:[], portfolio:[], simulatedTrades:[], ...(state || {})};
    await atomicWriteJson(file, value);
    return {ok:true, file};
  } catch (error) {
    appendLogLine({type:'error', action:'user_state_save_failed', message:'用户数据保存失败', stack:error.stack || String(error)});
    return {ok:false, error:error.message || String(error)};
  }
});

ipcMain.handle('explain-stock-with-ai', async (_event, request = {}) => {
  const apiKey = process.env.STOCK_AI_API_KEY || '';
  const endpoint = process.env.STOCK_AI_BASE_URL || '';
  const model = process.env.STOCK_AI_MODEL || '';
  if (!apiKey || !endpoint || !model) return {configured:false, message:'未配置 STOCK_AI_API_KEY、STOCK_AI_BASE_URL 和 STOCK_AI_MODEL'};
  const evidence = (request.evidence || []).filter(item => item?.id && item?.text).slice(0, 30);
  if (!request.analysisId || !evidence.length) return {configured:true, ok:false, message:'当前快照没有可引用证据'};
  const prompt = buildAiEvidencePrompt({...request, evidence});
  try {
    const response = await postJson(endpoint, {model, temperature:0.1, messages:[
      {role:'system', content:'你是A股研究解释器。只能重述输入证据并标注证据编号，不新增事实，不承诺收益，不给确定目标价。'},
      {role:'user', content:prompt}
    ]}, {Authorization:`Bearer ${apiKey}`});
    const text = response?.choices?.[0]?.message?.content || '';
    const validation = validateAiExplanation(text, evidence.map(item => item.id));
    if (!validation.valid) return {configured:true, ok:false, message:`AI输出证据校验失败${validation.unsupported.length ? `：${validation.unsupported.join('、')}` : ''}`};
    appendLogLine({type:'success', action:'ai_explanation', message:'AI证据解释完成', detail:{code:request.code,analysisId:request.analysisId,model}});
    return {configured:true, ok:true, text, model, analysisId:request.analysisId};
  } catch (error) {
    appendLogLine({type:'error', action:'ai_explanation_failed', message:'AI证据解释失败', detail:{code:request.code,analysisId:request.analysisId,model}, stack:error.stack || String(error)});
...
module.exports = {
  ...
  summarizeEastmoneyFundHistory,
  formatBusinessProducts,
  mapDataCenterCompanyProfile,
  selectSectorMemberBoards,
  mergeFundFlowSnapshot,
